...
def test_softmax(M=128, N=128):
    x = torch.randn((N, N), dtype=torch.bfloat16, device='cuda:0',
                    requires_grad=True)
...
def test_mla(B=2, L=4096, H=16, causal=True, hpc=False, safe=True, coef=1.0,
             clip_value=None, bench=False):
    ...
    output_ref, lse_ref, max_logits_ref = torch_attn(q, k, v, causal=causal,
                                                     hp=True)
    ...
    output, lse, max_logits = triton_mla_forward(q, k, v, causal=causal,
                                                 safe=safe,
    ...
    if bench:
        ...
        benchmark_func(triton_mla_forward, q, k, v, causal=causal, safe=safe,
...
def test_varlen_mla(LS=[2048, 4096], H=16, causal=True, hpc=False, safe=True,
                    coef=1.0, clip_value=None, pad=False, bench=False):
    dtype = torch.bfloat16
    device = 'cuda:0'
    if pad:
        cu_seqlens = torch.cumsum(torch.tensor([0] + LS, device=device), 0)
        LS = [x + 7 for x in LS]
        padded_cu_seqlens = torch.cumsum(torch.tensor([0] + LS, device=device),
                                         0)
    else:
        cu_seqlens = torch.cumsum(torch.tensor([0] + LS, device=device), 0)
        padded_cu_seqlens = None
    ...
    L = sum(LS)
    q = (torch.randn((L, H, 192), device=device,
                     dtype=dtype) * coef).requires_grad_()
    ...
    output_ref, lse_ref, max_logit_ref = torch_varlen_attn(q, k, v, cu_seqlens,
                                                           causal=causal,
                                                           hp=True)
    output_ref.backward(g, retain_graph=False)
    gq_ref = q.grad
    ...
    output, lse, max_logits = triton_varlen_mla_forward(q, k, v, cu_seqlens,
                                                        max_q_length,
                                                        causal=causal,
                                                        safe=safe,
                                                        clip_value=clip_value)
    ...
    if clip_value is None and safe:
        output_check(max_logit_ref, max_logits, atol=0.01, rtol=0.03,
                     name='max_logits')

    gq, gk, gv = triton_varlen_mla_backward(g, output, q, k, v, lse, max_logits,
    ...
    if bench:
        ref_flops = sum(
            [L * L * H * (192 + 128) * (1 if causal else 2) for L in LS])
        benchmark_func(triton_varlen_mla_forward, q, k, v, cu_seqlens,
                       max_q_length,
                       causal=causal, safe=safe,
                       clip_value=clip_value, ref_flops=ref_flops)
        ref_flops = sum(
            [L * L * H * (192 + 128 * 2 + 192 * 2) * (1 if causal else 2) for L
             in LS])
        benchmark_func(triton_varlen_mla_backward, g, output, q, k, v, lse,
                       max_logits,
                       cu_seqlens, max_q_length,
                       padded_cu_seqlens=padded_cu_seqlens,
                       causal=causal, hpc=hpc, safe=safe,
...
def test_fp8_mla(B=2, L=4096, H=16, causal=True, hpc=False, quant_value=False,
                 bench=False):
    dtype = torch.bfloat16
    device = 'cuda:0'
    q = torch.randn((B, L, H, 192), device=device, dtype=dtype,
                    requires_grad=True)
    k = torch.randn((B, L, H, 192), device=device, dtype=dtype,
                    requires_grad=True)
    v = torch.randn((B, L, H, 128), device=device, dtype=dtype,
    ...
    output, lse, max_logits = triton_fp8_mla_forward(q_q, k_q,
    ...
    if bench:
        ref_flops = B * L * L * H * (192 + 128) * (1 if causal else 2)
        benchmark_func(triton_fp8_mla_forward, q_q, k_q,
                       v_q if quant_value else v, q_s, k_s,
                       vs=v_s if quant_value else None, causal=causal,
                       ref_flops=ref_flops)
...
if __name__ == "__main__":
    ...
    test_varlen_mla(LS=[8192], H=64, causal=True, hpc=False, safe=False, coef=1.0, clip_value=None, pad=False, bench=False)
    test_varlen_mla(LS=[8192], H=64, causal=True, hpc=False, safe=True, coef=1.0, clip_value=100.0, pad=False, bench=False)
    test_varlen_mla(LS=[8192], H=64, causal=True, hpc=False, safe=True, coef=1.0, clip_value=None, pad=True, bench=False)
    ...
    test_fp8_mla(B=1, L=8192, H=64, causal=True, hpc=False, quant_value=False, bench=False)
