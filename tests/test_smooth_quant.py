...
def torch_split_transpose_smooth_quant(x_split, smooth_scale_split, round_scale=False, reverse=True):
    assert reverse
    x_qs = []
    x_scales = []
    for i, x_ in enumerate(x_split):
        ...
        M, N = x_.shape
        if M % 32 != 0:
            x_ = torch.cat([x_, torch.zeros((32 - M % 32, N),
                                            dtype=x_.dtype,
                                            device=x_.device)], 0)
            smooth_scale = torch.cat([smooth_scale, torch.zeros((32 - M % 32, ),
                                            dtype=torch.float32,
                                            device=x_.device)], 0)
            M = (M + 31)//32*32
...
def test_triton_smooth_quant(M=4096, N=4096, bench=False):
    ...
    smooth_scale = torch.randn((N,), device=device,
                               dtype=torch.float32).abs() + 1.0
    round_scale = False
    rtol = 2 if round_scale else 0.125
    x_q_ref, scales_ref = torch_smooth_quant(x, smooth_scale,
                                                         reverse=False,
                                                         round_scale=round_scale)

    x_q, x_scale = triton_smooth_quant(x, smooth_scale,
                                               reverse=False,
                                               round_scale=round_scale)
...
def test_triton_transpose_rescale_smooth_quant(M=4096, N=4096,
    ...
    y_q, y_scale = triton_smooth_quant(y, org_smooth_scale,
                                               reverse=True,
                                               round_scale=round_scale)

    yt_gt, yt_scale_gt = torch_smooth_quant(y.t(),
                                                        transpose_smooth_scale,
                                                        reverse=True,
                                                        round_scale=round_scale)
    ...
    if bench:
        ref_bytes = M * N * 3
        benchmark_func(triton_transpose_rescale_smooth_quant, y_q,
                                                           org_smooth_scale,
                                                           y_scale,
                                                           transpose_smooth_scale,
                                                           reverse=True,
...
def test_batch_smooth_quant(M=4096, N=4096, n_experts=32, topk=8,
    ...
    x_q, x_scale = triton_batch_smooth_quant(x, smooth_scales,
                                                     token_count_per_expert,
                                                     reverse=False,
                                                     round_scale=round_scale)

    x_split = torch.split(x, token_count_per_expert_list)
    x_q_ref, x_scale_ref = torch_split_smooth_quant(x_split,
                                                                smooth_scales)
    ...
    if bench:
        ...
        benchmark_func(triton_batch_smooth_quant, x, smooth_scales,
                       ...
                       round_scale=round_scale, ref_bytes=ref_bytes,
                       ref_time=ref_time)
        benchmark_func(triton_batch_smooth_quant, x, smooth_scales,
                       token_count_per_expert, reverse=False,
                       round_scale=round_scale,
...
def test_batch_transpose_smooth_quant(M=4096, N=4096, n_experts=32, topk=8,
                                   round_scale=False, bench=False):
    device = 'cuda:0'
    ...
    x = torch.randn((sum(token_count_per_expert_list), N), dtype=torch.bfloat16,
                    device=device)
    smooth_scales = 1 + torch.rand((sum(token_count_per_expert_list), ), device=device,
                                        dtype=torch.float32)

    x_q, x_scale = triton_batch_transpose_smooth_quant(x, smooth_scales,
                                                     token_count_per_expert,
    ...
    x_q_ref, x_scale_ref  = torch_split_transpose_smooth_quant(x_split,
                                                                smooth_scale_split,
                                                                round_scale=round_scale,
                                                                reverse=True)
    rtol = 2 if round_scale else 0.125
    ...
    output_check(x_scale_ref.float(), x_scale.float(),
                 'batch_transpose_smooth_quant.scale')

    if bench:
        ref_bytes = sum(token_count_per_expert_list) * N * 3
        benchmark_func(triton_batch_transpose_smooth_quant, x, smooth_scales,
                                                     token_count_per_expert,
                                                     token_count_per_expert_list,
                                                     reverse=True,
                                                     round_scale=round_scale,
...
if __name__ == '__main__':
    ...
    test_batch_smooth_quant(M=4096, N=4096, n_experts=32, topk=8,
                            round_scale=False, bench=False)

    test_batch_transpose_smooth_quant(M=4096, N=4096, n_experts=32, topk=8,
                                   round_scale=False, bench=False)
