...
def torch_group_topk_score(
    ...
    if expert_bias is not None:
        ...
        scores_for_routing = (
            scores
            + expert_bias
            - torch.arange(0, num_experts, device=logits.device).to(torch.float64)
            * 1e-12
        )
        _, top_indices = group_limited_topk(
            scores_for_routing, topk, num_tokens, num_experts, num_groups, group_topk
        )
    ...
    topk_masked_gates = torch.zeros_like(logits, dtype=torch.float64).scatter(
        1, top_indices, probs
    )
    topk_map = (
        torch.zeros_like(logits, dtype=torch.float64)
        .int()
        .scatter(1, top_indices, 1)
        .bool()
    )
...
def test_topk(M=4096, B=1, N=256, k=8, equal=False, bench=False):
    ...
    if equal:
        xd = x.to(torch.float64) * (
            1 - torch.arange(0, N, device=device).to(torch.float64) * 1e-12
        )
    ...
    if bench:
        ref_time = benchmark_func(torch.topk, x, k, ref_bytes=M * N * 4)
        benchmark_func(
            triton_topk_forward, x, k, ref_bytes=M * N * 4, ref_time=ref_time
        )
        benchmark_func(
            triton_topk_backward, index_ref.float(), index, N, ref_bytes=M * N * 4
        )


def test_group_topk_score(
    M=4096,
    N=256,
    k=8,
    num_groups=32,
    ...
    prob_ref, map_ref, count_ref = torch_group_topk_score(
        x,
    ...
    prob, maps, count = triton_group_topk_score_forward(
        x,
        k,
        expert_bias=expert_bias,
        num_groups=num_groups,
    ...
    grad = triton_group_topk_score_backward(
        map_ref.float() * dy, x, maps, scaling_factor=scaling_factor
    )
    ...
    prob, maps, count = group_topk_score(
        x,
        k,
        expert_bias=expert_bias,
        num_groups=num_groups,
