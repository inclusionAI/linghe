...
def test_triton_softmax_cross_entropy(M=4096, N=157184, coef=1.0, grad_coef=1.0,
    ...
    if select:
        ...
        targets = torch.randint(0, N * group_size, (M,), dtype=torch.long,
                                device=device)
    ...
    dist.all_gather_into_tensor(global_logits, local_logits.detach(),
                                group=group)
    global_logits = torch.reshape(torch.permute(global_logits, (1, 0, 2)), (
    M, group_size * N)).contiguous().requires_grad_()

    global_targets = torch.empty((group_size, M), dtype=torch.long,
                                 device=device)
    dist.all_gather_into_tensor(global_targets, targets, group=group)
    global_targets = global_targets[0]

    local_output_grad = torch.randn((M,), dtype=torch.float32,
                                    device=device) * grad_coef
    global_output_grad = torch.empty((group_size, M), dtype=torch.float32,
                                     device=device)
    dist.all_gather_into_tensor(global_output_grad, local_output_grad,
                                group=group)
    ...
    loss, sum_exp, max_logit = triton_parallel_softmax_cross_entropy_forward(
        ...
        group,
        ignore_index=ignore_index)
    output_check(loss_ref, loss, name=f'ref_loss:{group_rank}', atol=1e-4,
                 rtol=1e-5)

    grad = triton_parallel_softmax_cross_entropy_backward(
        local_logits.detach().clone(), global_targets, sum_exp,
        max_logit,
        global_output_grad,
        group,
        ignore_index=ignore_index,
    ...
    output_check(grad_ref[:, group_rank * N:(group_rank + 1) * N], grad,
                 name=f'grad:{group_rank}', digest=10)
    ...
    if bench:
        benchmark_func(torch_cross_entropy, global_logits.requires_grad_(),
                       global_targets,
                       ref_bytes=M * N * 2)
        benchmark_func(triton_parallel_softmax_cross_entropy_forward,
                       local_logits, global_targets, group,
                       ignore_index=ignore_index,
                       ref_bytes=M * N * 2)
        benchmark_func(loss_ref.backward, global_output_grad, retain_graph=True,
                       ref_bytes=M * N * 4)
        benchmark_func(triton_parallel_softmax_cross_entropy_backward,
                       local_logits.detach().clone(), global_targets,
                       sum_exp, max_logit, global_output_grad, group,
...
if __name__ == '__main__':
    ...
    test_triton_softmax_cross_entropy(M=8192, N=157184, coef=1.0, grad_coef=1.0,
                                      fill=True, inplace=False, group=pg,
                                      bench=False)
    test_triton_softmax_cross_entropy(M=8192, N=157184, coef=1.0, grad_coef=1.0,
                                      fill=True, inplace=True, group=pg,
                                      bench=False)
