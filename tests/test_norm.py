...
def test_rmsnorm(M=4096, N=4096, bench=False):
    ...
    if bench:
        ...
        benchmark_func(
            triton_rms_norm_backward, dy, x, weight, rms=rms, ref_bytes=M * N * 3
        )
...
def test_rmsnorm_and_block_quant(M=4096, N=4096, bench=False):
    ...
    # blockwise
    q_ref, scale_ref, rms_ref, qt_ref, scale_t_ref = torch_rms_and_block_quant_forward(
        x, weight, round_scale=True
    )

    q, scale, rms, _, _ = triton_rms_norm_and_block_quant_forward(
...
def test_rms_norm_fp32_gemm_block_quant_forward(M=8192, N=256, K=2048, bench=False):
    ...
    # blockwise
    y_ref, logit_ref, q_ref, scale_ref, qt_ref, scale_t_ref = (
        torch_rms_gemm_block_quant_forward(
            x, norm_weight, route_weight, round_scale=round_scale
        )
    )
    y, rms, logit, q, scale, q_t, scale_t = (
        triton_rms_norm_fp32_gemm_block_quant_forward(
            x, norm_weight, route_weight, round_scale=round_scale, output_mode=0
        )
    )
    ...
    if bench:
        ...
        benchmark_func(
            split_rms_gemm_block_quant_forward,
            x,
            norm_weight,
            route_weight,
            round_scale=round_scale,
...
if __name__ == "__main__":
    ...
    test_rmsnorm_and_block_quant(M=16384, N=1536, bench=False)
