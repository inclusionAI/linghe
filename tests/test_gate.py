...
def test_group_rms_norm_gate(
    ...
    if transpose:
        ...
        grad_output = (
            torch.randn(length, bs, dim, dtype=dtype, device=device) * grad_coef
        )
    else:
        gate = (
            torch.randn(bs, length, dim, dtype=dtype, device=device) * coef
        ).requires_grad_()
        grad_output = (
            torch.randn(bs, length, dim, dtype=dtype, device=device) * grad_coef
        )
...
if __name__ == "__main__":
    ...
    test_group_rms_norm_gate(
        bs=2, length=4096, dim=1536, group_size=4, transpose=True, bench=False
    )
    test_group_rms_norm_gate(
        bs=2,
        length=4096,
