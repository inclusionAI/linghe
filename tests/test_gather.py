...
def test_batch_mxfp8_permute_with_indices(M=16384, N=2048, n_experts=32, topk=2, bench=False):
    device = 'cuda:0'
    logits = torch.randn((M, n_experts), dtype=torch.float32,
                        device=device) ** 3
    logits[:,0] -= 1000
    ...
    x_q_ref, x_s_ref, xt_q_ref, xt_s_ref, p_ref = torch_batch_mxfp8_permute_with_indices(x, 
                                      indices,
                                      probs,
                                      token_count_per_expert_list)
    ...
    if bench:
        ...
        benchmark_func(triton_batch_mxfp8_permute_with_indices, x, token_count_per_expert,
                                       indices,
                                       token_count_per_expert_list,
                                       probs=probs,
                                       ref_bytes=num_out_tokens * N * 4)
        ...
        benchmark_func(triton_permute_with_mask_map, x, None, probs,
                       ...
                       ref_bytes=num_out_tokens * N * 4)
        xs = x[indices]
        benchmark_func(triton_batch_mxfp8_quant, xs, token_count_per_expert,  token_count_per_expert_list,
                       ref_bytes=num_out_tokens * N * 4)
