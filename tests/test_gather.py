...
def torch_smooth_permute_with_indices(
    x, indices, smooth_scales, token_count_per_expert_list, probs=None, round_scale=True
):
    M, N = x.shape
    q_refs = []
    scale_refs = []
...
def torch_smooth_fused_permute_with_indices(
    grad_data,
    grad_scale,
    indices,
    org_smooth_scales,
...
def torch_batch_transpose_smooth_permute_with_indices(
    x, smooth_scales, indices, token_count_per_expert_list, round_scale=True
):
    M, DIM = x.shape
    q_refs = []
    scale_refs = []
    ...
    for i, c in enumerate(token_count_per_expert_list):
        c = token_count_per_expert_list[i]
        if c == 0:
            y_scale = torch.zeros((DIM,), dtype=torch.float32, device=x.device)
...
def torch_batch_transpose_smooth_fused_permute_with_indices(
    x_q,
    x_scale,
    org_smooth_scale,
    smooth_scales,
...
def torch_batch_block_pad_permute_with_indices(
    x, indices, probs, token_count_per_expert_list, round_scale=True
):
    M, DIM = x.shape
    if M == 0:
        device = x.device
    ...
    for i, c in enumerate(token_count_per_expert_list):
        ...
        y_q, y_scale, yt_q, yt_scale = torch_blockwise_quant(
            y, round_scale=round_scale, padding=True
        )
        q_refs.append(y_q.view(-1))
...
@pytest.mark.parametrize(
    "M,n_experts,topk,bias",
    [
        (4098, 32, 2, 0.0),
    ],
)
def test_make_id_map(M, n_experts, topk, bias, benchmark, ep=4):
    ...
    row_id_map, resort_row_id_map = triton_make_chunk_sort_map(
        num_global_tokens_per_local_expert, token_per_expert
    )
    row_id_map_torch, resort_row_id_map_torch = torch_make_chunk_sort_map(
...
def test_triton_permute_with_mask_map(M, N, n_experts, topk, benchmark):
    device = "cuda:0"
    dtype = torch.bfloat16
...
def test_batch_smooth_permute_with_indices(
    M, N, n_experts, topk, benchmark, round_scale=True
):
    device = "cuda:0"
    dtype = torch.bfloat16
    ...
    q_ref, scale_ref, prob_ref = torch_smooth_permute_with_indices(
        grad_output,
        indices,
        smooth_scales,
        token_count_per_expert_list,
    ...
    y_q, y_scale, prob = triton_batch_smooth_permute_with_indices(
        grad_output,
        smooth_scales,
...
def test_batch_transpose_smooth_permute_with_indices(M, N, n_experts, topk, benchmark):
    device = "cuda:0"
    logits = torch.randn((M, n_experts), dtype=torch.float32, device=device) ** 3
    logits[:, 0] -= 1000
    logits[:, 2] -= 100
...
def test_batch_smooth_fused_permute_with_indices(
    M, N, n_experts, topk, benchmark, round_scale=True
):
    device = "cuda:0"
    dtype = torch.bfloat16
    smooth_scales = 1 + 1 * torch.rand(
        (n_experts, N), device=device, dtype=torch.float32
    )
    org_smooth_scales = 1 + 1 * torch.rand(
        (n_experts, N), device=device, dtype=torch.float32
    )
    logits = torch.randn((M, n_experts), dtype=torch.float32, device=device)
    probs, mask_map, token_count_per_expert, indices, row_id_map = torch_make_indices(
    ...
    grad_data = torch.randn((M, N), dtype=dtype, device=device).to(torch.float8_e4m3fn)
    grad_scale = 1 + torch.rand((M,), dtype=torch.float32, device=device)
    q_ref, scale_ref = torch_smooth_fused_permute_with_indices(
        grad_data,
        grad_scale,
        indices,
        org_smooth_scales,
    ...
    y_q, y_scale = triton_batch_smooth_fused_permute_with_indices(
        grad_data,
        grad_scale,
        org_smooth_scales,
        smooth_scales,
...
def test_batch_transpose_smooth_fused_permute_with_indices(
    M, N, n_experts, topk, benchmark
):
    device = "cuda:0"
    if True:
        logits = torch.randn((M, n_experts), dtype=torch.float32, device=device) ** 3
    ...
    x_q_ref, x_scale_ref = torch_batch_transpose_smooth_fused_permute_with_indices(
        x,
        scale,
        org_smooth_scale,
        smooth_scales,
    ...
    x_q, x_scale = triton_batch_transpose_smooth_fused_permute_with_indices(
        x,
        scale,
        org_smooth_scale,
        smooth_scales,
    ...
    benchmark(
        torch_batch_transpose_smooth_fused_permute_with_indices,
        x,
        scale,
        org_smooth_scale,
        smooth_scales,
...
def test_batch_block_pad_permute_with_indices(M, N, n_experts, topk, benchmark):
    device = "cuda:0"
    logits = torch.randn((M, n_experts), dtype=torch.float32, device=device) ** 3
    logits[:, 0] -= 1000
    logits[:, 2] -= 100
    ...
    row_id_map, pad_indices = triton_make_row_id_map_and_index(
        mask_map, num_out_tokens, multiple_of=16
    )

    x = torch.randn((M, N), dtype=torch.bfloat16, device=device)

    x_q_ref, x_s_ref, xt_q_ref, xt_s_ref, p_ref = (
        torch_batch_block_pad_permute_with_indices(
            x, indices, probs, token_count_per_expert_list, round_scale=True
        )
    )

    x_q, x_s, xt_q, xt_s, p = triton_batch_block_pad_permute_with_indices(
        x,
        token_count_per_expert,
        pad_indices,
        token_count_per_expert_list,
...
def test_batch_mxfp8_permute_with_indices(M, N, n_experts, topk, benchmark):
    device = "cuda:0"
    logits = torch.randn((M, n_experts), dtype=torch.float32, device=device) ** 3
    logits[:, 0] -= 1000
    logits[:, 2] -= 100
