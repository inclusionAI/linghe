...
def torch_fp64_matmul(x, w):
    return torch.nn.functional.linear(x.to(torch.float64), w.to(torch.float64)).to(
        torch.float32
    )
...
def test_fp32_matmul(M, N, K, benchmark):
    ...
    ref_flops = 2 * M * N * K
    ref_time = benchmark(
        torch_fp32_matmul, x, w, ref_bytes=ref_bytes, ref_flops=ref_flops
    )
    benchmark(
        triton_fp32_gemm,
        x,
...
def test_split_fp32_matmul(M, N, K, benchmark):
    dtype = torch.bfloat16
    device = "cuda:0"
...
@pytest.mark.skipif(
    _Version(triton.__version__) <= _Version("3.4.0"),
    reason="requires Triton > 3.4.0",
)
@pytest.mark.parametrize(
    "M,N,K",
    [
...
def test_tma_fp32_matmul(M, N, K, benchmark):
    dtype = torch.bfloat16
    device = "cuda:0"
...
@pytest.mark.parametrize(
    "B,M,N,K,impl",
    [
        (2, 8192, 128, 1536, "native"),
        (2, 8192 - 32, 128, 1536, "native"),
        (2, 8192, 128, 1536, "tma"),
        (2, 8192, 128, 1536, "split"),
    ],
)
def test_fp32_facade(B, M, N, K, impl, benchmark):
    # M, N, K = 4096, 256, 8192
    dtype = torch.bfloat16
    device = "cuda:0"
    n_repeat = 100
