...
def test_triton_channelwise_gemm(M=4096, N=4096, K=4096, bench=False):
    ...
    y_ref = (x_q.float()*x_scales[:,None])@(w_q.float()*w_scales[:,None]).t()
    ...
    if bench:
        ...
        benchmark_func(scaled_gemm_and_update, x_q, w_q, x_scales, w_scales, c=y_bf16, 
                    accum=False, n_repeat=n_repeat, ref_flops=ref_flops)

        benchmark_func(scaled_gemm_and_update, x_q, w_q, x_scales, w_scales, c=y_bf16, 
                    accum=True, n_repeat=n_repeat, ref_flops=ref_flops)
        benchmark_func(scaled_gemm_and_update, x_q, w_q, x_scales, w_scales, c=y_fp16,
                    accum=True, n_repeat=n_repeat, ref_flops=ref_flops)
        benchmark_func(scaled_gemm_and_update, x_q, w_q, x_scales, w_scales, c=y_fp32,
                    accum=True, n_repeat=n_repeat, ref_flops=ref_flops)
