...
def test_silu_and_smooth_quant(M=4096, N=4096, coef=1.0, grad_coef=1.0, bench=False):
    ...
    x = (x * coef).clone().detach().requires_grad_()
    grad_output = (
        torch.randn((M, N // 2), dtype=torch.bfloat16, device="cuda:0") * grad_coef
    )
    ...
    y_q, y_scale, y_maxs = triton_silu_and_smooth_quant_forward(
        x, smooth_scale=smooth_scale, round_scale=round_scale, calibrate=True
    )
...
def test_silu_and_block_quant(M=4096, N=4096, coef=1.0, grad_coef=1.0, bench=False):
    ...
    dx_q_ref, dx_scale_ref, dxt_q_ref, dxt_scale_ref = (
        torch_silu_and_block_quant_backward(grad_output, x, round_scale=round_scale)
    )
    dx_q, dx_scale, dxt_q, dxt_scale = triton_silu_and_block_quant_backward(
        grad_output, x, round_scale=round_scale
    )
...
def test_triton_batch_weighted_silu_and_smooth_quant(
    M=1024, N=4096, n_experts=32, coef=1.0, grad_coef=1.0, bench=False
):
    count_list = [
    ...
    smooth_scales = (
        1 + torch.rand((n_experts, N // 2), dtype=torch.float32, device="cuda:0") * 10
    )

    grad_output = (
        torch.randn((bs, N // 2), dtype=torch.bfloat16, device="cuda:0") * grad_coef
    )
    grad_smooth_scales = (
...
def test_triton_batch_weighted_silu_and_block_quant(
    M=1024, N=4096, n_experts=32, bench=False, coef=1.0, grad_coef=1.0
):
    count_list = [
        random.randint(M // 2, M // 2 * 3) // 16 * 16 for _ in range(n_experts)
    ]
    ...
    x_q, x_scale, xt_q, xt_scale = triton_batch_weighted_silu_and_block_quant_forward(
        x, weight, counts, count_list, round_scale=round_scale, output_mode=0
    )
    output_check(x_q_ref, x_q, "block.q", rtol=rtol)
    output_check(x_scale_ref, x_scale.view(-1), "block.scale")

    x_q, x_scale, xt_q, xt_scale = triton_batch_weighted_silu_and_block_quant_forward(
        x, weight, counts, count_list, round_scale=round_scale, output_mode=1
    )
    output_check(xt_q_ref, xt_q.view(-1), "block.qt", rtol=rtol)
    output_check(xt_scale_ref, xt_scale.view(-1), "block.t_scale")
...
if __name__ == "__main__":
    ...
    test_silu_and_block_quant(
        M=4096, N=1536 * 8, coef=100.0, grad_coef=100.0, bench=False
    )
    ...
    test_triton_batch_weighted_silu_and_block_quant(
        M=0, N=1536, n_experts=32, bench=False
    )
    test_triton_batch_weighted_silu_and_block_quant(
        M=2048, N=8192, n_experts=32, bench=False
    )
    test_triton_batch_weighted_silu_and_block_quant(
        M=12080, N=1536, n_experts=32, coef=100.0, grad_coef=100.0, bench=False
    )
    test_triton_batch_weighted_silu_and_block_quant(
        M=12080, N=1536, n_experts=32, coef=0.0, grad_coef=0.0, bench=False
    )
