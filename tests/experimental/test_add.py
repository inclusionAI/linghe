...
from linghe.experimental.add import triton_inplace_add_warp_specialized
...
def test_triton_inplace_add(M=4096, N=4096, bench=False):
    ...
    out = outputs.clone()
    triton_inplace_add_warp_specialized(out, x)
    out_ref = outputs + x
    output_check(out_ref, out, 'sum')
...
if __name__ == '__main__':
    test_triton_inplace_add(M=4096, N=4096, bench=True)
