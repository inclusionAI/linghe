...
def test_embedding(B=2, M=4096, V=150000, D=4096, transpose=False, bench=False):
    ...
    if bench:
        ...
        ref_time = benchmark_func(y_ref.backward, dy, retain_graph=True, ref_bytes=ref_bytes)
...
def test_fused_embedding(B=2, M=4096, V=150000, D=4096, use_main_grad=True, transpose=False, bench=False):
    dtype = torch.bfloat16
    device = 'cuda:0'
    ...
    if bench:
        ...
        benchmark_func(fused_accumulation_embedding_lookup, input_ids, weights, grad_name=grad_name,
                       ref_time=ref_time, ref_bytes=ref_bytes)
