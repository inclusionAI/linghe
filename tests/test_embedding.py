...
def test_scan(M=4096, bench=False):
    ...
    accum_counts_ref = torch.cumsum(
        torch.tensor([0] + unique_counts_ref.tolist(), device=unique_counts_ref.device),
        0,
    )
...
def test_embedding(B=2, M=4096, V=150000, D=4096, transpose=False, bench=False):
    ...
    if bench:
        ref_bytes = B * M * D * 4
        ref_time = benchmark_func(embedding.forward, input_ids, ref_bytes=ref_bytes)
        benchmark_func(
            embedding_lookup, input_ids, weights, ref_time=ref_time, ref_bytes=ref_bytes
        )

        ref_time = benchmark_func(
            y_ref.backward, dy, retain_graph=True, ref_bytes=ref_bytes
        )
        benchmark_func(
            triton_atomic_embedding_backward,
            dy,
        ...
        benchmark_func(
            y.backward, dy, retain_graph=True, ref_time=ref_time, ref_bytes=ref_bytes
        )


def test_fused_embedding(
    B=2, M=4096, V=150000, D=4096, use_main_grad=True, transpose=False, bench=False
):
    dtype = torch.bfloat16
    device = "cuda:0"
...
if __name__ == "__main__":
    ...
    test_fused_embedding(B=1, M=8192, V=150000, D=8192, transpose=False, bench=False)
    test_fused_embedding(B=1, M=4096, V=150000, D=8192, transpose=False, bench=False)
    test_fused_embedding(B=2, M=4096, V=150000, D=8192, transpose=True, bench=False)
    test_fused_embedding(B=0, M=4096, V=150000, D=8192, transpose=True, bench=False)
