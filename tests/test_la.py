...
def make_varlen_input(qo_heads=16, kv_heads=16, dim=128, qls=[1024,1024], kls=[1024,1024]):
...
def test_la(bs=1, length=4096, qo_heads=16, kv_heads=16, dim=128, digest=False, bench=False):
    device = torch.device('cuda:0')
    dtype = torch.bfloat16
    ...
    if max_decay_scale < 0.1:
        dq, dk, dv = triton_fused_lightning_attention_backward(g, q, k, v, state, decay_scales)
    ...
    if bench:
        ...
        benchmark_func(triton_lightning_attention_forward, q, k, v, decay_scales, ref_bytes=ref_bytes)
        benchmark_func(triton_lightning_attention_backward, g, q, k, v, decay_scales, ref_bytes=ref_bytes)
        benchmark_func(triton_fused_lightning_attention_backward, g, q, k, v, state, decay_scales, ref_bytes=ref_bytes)
...
if __name__ == '__main__':
    test_la(bs=1, length=8192, qo_heads=64, kv_heads=64, dim=128, digest=False, bench=True)