...
@pytest.mark.parametrize(
    "bs,length,qo_heads,kv_heads,dim",
    [
        (1, 8192, 64, 64, 128),
    ],
)
def test_la(bs, length, qo_heads, kv_heads, dim, benchmark):
    device = torch.device("cuda:0")
    dtype = torch.bfloat16
    ...
    benchmark(
        triton_lightning_attention_forward, q, k, v, decay_scales, ref_bytes=ref_bytes
    )
    benchmark(
        triton_lightning_attention_backward,
        g,
