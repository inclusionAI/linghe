...
def torch_batch_blockwise_quant(x, 
    ...
    for i, c in enumerate(token_count_per_expert_list):
        ...
        y_q, y_scale, yt_q, yt_scale = torch_blockwise_quant(y, round_scale=round_scale, padding=False)
...
def test_block_quant(M=8192,N=4096,bench=False):
    ...
    if bench:
        benchmark_func(triton_block_quant, x, 
                                       round_scale=True,
                                       ref_bytes=M * N * 4)
...
def test_blockwise_quant(M=8192,N=4096,bench=False):
    ...
    x_q_ref, x_s_ref, xt_q_ref, xt_s_ref = torch_blockwise_quant(x, round_scale=True, padding=False)
    ...
    if bench:
        benchmark_func(triton_blockwise_quant, x, 
                                       round_scale=True,
                                       ref_bytes=M * N * 4)
...
def test_batch_block_quant(M=16384, N=2048, n_experts=32, topk=2, bench=False):
    ...
    x_q_ref, x_s_ref, xt_q_ref, xt_s_ref = torch_batch_blockwise_quant(x, 
                                      token_count_per_expert_list,
                                      round_scale=True)

    x_q, x_s, xt_q, xt_s = triton_batch_blockwise_quant(x, 
    ...
    if bench:
        benchmark_func(triton_batch_blockwise_quant, x, token_count_per_expert,
                                       token_count_per_expert_list,
                                       round_scale=True,
                                       ref_bytes=M * N * 4)
