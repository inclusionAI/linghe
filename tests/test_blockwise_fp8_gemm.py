...
def test_blockwise_gemm(M=4096, N=4096, K=4096, bench=False):
    ...
    y_ref = x_dq @ w_dq.t()
    y = triton_blockwise_fp8_gemm(x_q, w_q, x_scales, w_scales,
    ...
    if bench:
        ...
        benchmark_func(triton_blockwise_fp8_gemm, x_q, w_q, x_scales, w_scales,
                       out_dtype=dtype, block_size=B,
                       n_repeat=n_repeat, ref_flops=ref_flops)


# def test_tilewise_gemm(M=4096, N=4096, K=4096, bench=False):
#     dtype = torch.bfloat16
#     device = 'cuda:0'
#     B = 64
...
#     y_ref = x_dq @ w_dq.t()
#     y = triton_tilewise_fp8_gemm(x_q, w_q, x_scales, w_scales,
#                            out_dtype=dtype, block_size=B)
...
#         benchmark_func(triton_tilewise_fp8_gemm, x_q, w_q, x_scales, w_scales,
#                        out_dtype=dtype, block_size=B,
#                        n_repeat=n_repeat, ref_flops=ref_flops)


if __name__ == '__main__':
    test_blockwise_gemm(M=4096, N=8192, K=2048, bench=False)
