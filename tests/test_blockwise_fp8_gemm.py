...
def test_triton_bb_gemm(M=4096, N=4096, K=4096, bench=False):
    ...
    y_ref = x_dq @ w_dq.t()
    y = triton_bb_fp8_gemm(x_q, w_q, x_scales, w_scales, out_dtype=dtype, block_size=B)
...
def test_triton_tt_gemm(M=4096, N=4096, K=4096, bench=False):
    ...
    y_ref = x_dq @ w_dq.t()
    y = triton_tt_fp8_gemm(x_q, w_q, x_scales, w_scales, out_dtype=dtype, block_size=B)
