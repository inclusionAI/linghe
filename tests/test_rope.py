...
def torch_mla_rope(q, kv, k_pos_emb, freqs, mscale=1.0, transpose=False):
    ...
    q_pos_emb = torch.cat([q_pos_emb[:, :, :, 0::2], q_pos_emb[:, :, :, 1::2]],
                          -1)
    k_pos_emb = torch.cat([k_pos_emb[:, :, :, 0::2], k_pos_emb[:, :, :, 1::2]],
                          -1)
...
def torch_varlen_mla_rope(qs, kvs, k_pos_embs, freqs, lengths, mscale=1.0,
                          cp_size=1, cp_rank=0):
    ...
    for i in range(B):
        ...
        for j in range(2):
            ...
            position_ids = p + torch.arange(lengths[i] // seg_size,
                                            device='cuda:0')[:, None]
            ...
            q_pos_emb, k_pos_emb = apply_rotary_pos_emb(q_pos_emb, k_pos_emb,
                                                        cos, sin,
                                                        position_ids)
...
def torch_qk_norm_and_half_rope(qkv, qw, kw, freqs, H=32,
                                h=4, eps=1e-6, interleaved=True,
                                transposed=True, silu=False):
    if transposed:
...
def torch_varlen_qk_norm_and_half_rope(qkvs, qw, kw, freqs, lengths, H=32, h=4,
    ...
    for i in range(B):
        ...
        for j in range(2):
            ...
            query, key, value = torch_qk_norm_and_half_rope(qkv, qw, kw, fs,
                                                            H=H,
                                                            h=h, eps=eps,
...
def test_qk_norm_and_half_rope(B=2, L=4096, H=32, h=8, D=128,
    ...
    qw = torch.nn.Parameter(torch.randn(D, dtype=dtype, device=device),
                            requires_grad=True)
    kw = torch.nn.Parameter(torch.randn(D, dtype=dtype, device=device),
                            requires_grad=True)
    freqs = rope_freqs(L, D // 2, rope_theta=rope_theta)
    freqs = torch.cat([freqs, freqs], -1)
...
def test_varlen_qk_norm_and_half_rope(lengths=[2048, 2048], H=32, h=4, dim=128,
    ...
    q_grad = torch.randn(sum(lengths) // cp_size, H, dim, dtype=dtype,
                         device=device)
    k_grad = torch.randn(sum(lengths) // cp_size, h, dim, dtype=dtype,
                         device=device)
    v_grad = torch.randn(sum(lengths) // cp_size, h, dim, dtype=dtype,
                         device=device)
    ...
    qo, ko, vo = triton_varlen_qk_norm_and_half_rope_forward(qkv, qw, kw, freqs,
                                                             cu_seqlens_q,
                                                             cu_seqlens_kv, H=H,
                                                             h=h,
                                                             interleaved=interleaved,
    ...
    dqkv, dqw, dkw = triton_varlen_qk_norm_and_half_rope_backward(q_grad,
                                                                  k_grad,
                                                                  v_grad,
                                                                  qkv, qw, kw,
                                                                  freqs,
    ...
    if bench:
        ...
        benchmark_func(triton_varlen_qk_norm_and_half_rope_forward, qkv, qw, kw,
                       freqs,
                       cu_seqlens_q, cu_seqlens_kv, interleaved=interleaved,
                       H=H, h=h,
                       silu=silu, mscale=mscale, cp_size=cp_size,
                       cp_rank=cp_rank,
                       ref_bytes=lbh * (
                                   64 * 2 + 256 * 2 + 64 * 2 + 192 * 2 + 128 * 2),
                       n_profile=0)
        benchmark_func(triton_varlen_qk_norm_and_half_rope_backward, q_grad,
                       k_grad, v_grad,
...
def test_mla_rope(B=2, L=4096, H=32, rope_theta=10000.0, transpose=False,
                  bench=False):
    dtype = torch.bfloat16
    device = 'cuda:0'
    ...
    q_ref, k_ref, v_ref = torch_mla_rope(q, kv, k_pos_emb, freqs,
                                         mscale=mscale, transpose=transpose)
    q_ref.backward(gradient=q_grad, retain_graph=True)
    k_ref.backward(gradient=k_grad, retain_graph=True)
    v_ref.backward(gradient=v_grad, retain_graph=True)
    dq_ref = q.grad
    ...
    qo, ko, vo = triton_mla_rope_forward(q.clone().detach(), kv, k_pos_emb,
                                         freqs, mscale=mscale,
                                         transpose=transpose)
    output_check(q_ref, qo, name='q')
    output_check(k_ref, ko, name='k')
    ...
    dq, dkv, dp = triton_mla_rope_backward(q_grad.clone().detach(), k_grad,
                                           v_grad, freqs, mscale=mscale,
...
def test_varlen_mla_rope(lengths=[2048, 2048], H=32, rope_theta=10000.0,
    ...
    kvc = torch.randn(sum(lengths) // cp_size, H, 256, dtype=dtype,
                      device=device).requires_grad_()
    k_pos_emb = torch.randn(sum(lengths) // cp_size, 576, dtype=dtype,
                            device=device)
    k_pos_embc = k_pos_emb[:, 512:].view(sum(lengths) // cp_size, 1,
                                         64).requires_grad_()
    ...
    mscale = 1.0
    q_ref, k_ref, v_ref = torch_varlen_mla_rope(qc, kvc, k_pos_embc, freqs,
                                                lengths,
                                                mscale=mscale, cp_size=cp_size,
                                                cp_rank=cp_rank)
    qo, ko, vo = triton_mla_rope_forward(qc.clone().detach(), kvc, k_pos_embc,
                                         freqs, mscale=mscale,
    ...
    qo_ref, ko_ref, vo_ref = torch_varlen_mla_rope(q_i, kv_i, k_pos_emb_i,
                                                   freqs, lengths,
                                                   mscale=mscale,
                                                   cp_size=cp_size,
                                                   cp_rank=cp_rank)
    ...
    if bench:
        lbh = sum(lengths) // cp_size * H
        benchmark_func(triton_mla_rope_forward, qc, kvc, k_pos_embc, freqs,
                       mscale=mscale,
