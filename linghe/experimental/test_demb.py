...
from linghe.experimental.demb import (
    triton_tp_embedding_lookup_forward,
...
def torch_tp_emb(input_ids, weights, group):
    ...
    mask = torch.logical_and(
        input_ids >= group_rank * V, input_ids < (group_rank + 1) * V
    )
...
def test_tp_emb(
    B=1, M=4096, N=157184, D=4096, coef=1.0, grad_coef=1.0, group=None, bench=False
):
    ...
    local_weights = torch.randn((N, D), dtype=dtype, device=device, requires_grad=False)
    ...
    global_weights = (
        torch.reshape(global_weights, (group_size * N, D)).contiguous().requires_grad_()
    )

    local_ids = torch.randint(
        0, N * group_size, (B, M), dtype=torch.long, device=device
    )
    ...
    local_grad = torch.randn((B, M, D), dtype=dtype, device=device, requires_grad=False)
    ...
    output = triton_tp_embedding_lookup_forward(
        global_ids,
        local_weights,
        hdl,
        group,
    ...
    if bench:
        benchmark_func(
            F.embedding, global_ids, global_weights, ref_bytes=M * D * group_size * 2
        )
        benchmark_func(
            torch_tp_emb,
            global_ids,
...
def test_sp_emb(
    ...
    output = triton_sp_embedding_lookup_forward(
        local_ids,
        local_weights,
        hdl,
        group,
...
if __name__ == "__main__":
    ...
    test_sp_emb(
        M=8192, N=157184, D=4096, coef=1.0, grad_coef=1.0, group=group, bench=True
    )
