...
def torch_tp_emb(input_ids, weights, group):
    ...
    ids = input_ids % V
    output = weights[ids]
    mask = torch.logical_and(input_ids >= group_rank * V, input_ids < (group_rank +1) * V)
...
def test_tp_emb(B=1, M=4096, N=157184, D=4096, coef=1.0, grad_coef=1.0, 
    ...
    dist.all_gather_into_tensor(global_weights, local_weights.detach(), group=group)
    global_weights = torch.reshape(global_weights, (group_size * N, D)).contiguous().requires_grad_()

    local_ids = torch.randint(0, N * group_size, (B, M), dtype=torch.long, device=device)
    global_ids = torch.empty((group_size, B, M), dtype=torch.long, device=device)
    ...
    local_weight_grad_ref = global_weight_grad_ref[group_rank*N:(group_rank+1)*N]
    ...
    output_check(output_ref, output, name=f'output:{group_rank}', atol=1e-4, rtol=1e-5)

    weight_grad = torch.zeros((N, D), dtype=torch.float32, device=device)
    triton_tp_embedding_lookup_backward(local_grad, global_ids, weight_grad.data_ptr(), N,  hdl, group, dtype=weight_grad.dtype)
    output_check(local_weight_grad_ref, weight_grad.to(dtype), name=f'grad:{group_rank}', atol=-1e-4, rtol=1e-2)
    ...
    if bench:
        ...
        benchmark_func(torch_tp_emb, global_ids, local_weights, group,
                       ref_bytes=M * D * group_size * 2)
        benchmark_func(triton_tp_embedding_lookup_forward, global_ids, local_weights, hdl, group,
                       ref_bytes=M * D * group_size * 2)
        benchmark_func(triton_tp_embedding_lookup_backward, local_grad, global_ids, 
                       weight_grad.data_ptr(), N,  hdl, group, dtype=weight_grad.dtype,
                       ref_bytes=M * D * group_size * 2)
...
def test_sp_emb(B=1, M=4096, N=157184, D=4096, coef=1.0, grad_coef=1.0, 
    ...
    weight_grad = torch.zeros((N, D), dtype=torch.float32, device=device)
    triton_sp_embedding_lookup_backward(local_grad, local_ids, weight_grad.data_ptr(), N,  hdl, group, dtype=weight_grad.dtype)
    output_check(local_weight_grad_ref, weight_grad.to(dtype), name=f'grad:{group_rank}', atol=0.02, rtol=0.03)
    ...
    if bench:
        ...
        #                ref_bytes=M * D * group_size * 2)
        benchmark_func(triton_sp_embedding_lookup_forward, local_ids, local_weights, hdl, group,
                       ref_bytes=M * D * group_size * 2)
        benchmark_func(triton_sp_embedding_lookup_backward, local_grad, local_ids, 
                       weight_grad.data_ptr(), N,  hdl, group, dtype=weight_grad.dtype,
                       ref_bytes=M * D * group_size * 2)
...
if __name__ == '__main__':
    ...
    test_sp_emb(M=8192, N=157184, D=4096, coef=1.0, grad_coef=1.0, group=group, bench=True)
