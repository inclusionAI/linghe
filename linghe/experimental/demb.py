...
def tp_embedding_lookup_forward_kernel(input_ids_ptr,
    ...
    if (input_id >= RANK * V) & (input_id < (RANK + 1) * V):
        ...
        buffer_ptr = tl.load(buffer_ptrs + input_id // V).to(tl.pointer_type(tl.bfloat16))
        buffer_ptr = tl.multiple_of(buffer_ptr, 16)
...
def tp_embedding_lookup_backward_kernel(grad_output_ptr,
    ...
    if (input_id >= RANK * V) & (input_id < (RANK + 1) * V):
        for i in range(count):
            ...
            bid = pos // L
            lid = pos % L
            g = tl.load(grad_output_ptr + bid * stride_0 + lid * stride_1 + tl.arange(0, DIM), mask=mask).to(tl.float32)
            outputs += g
        symm_mem_sync(
                signal_ptrs,
                None,
        ...
        for j in range(SIZE):
            if j != RANK:
                buffer_ptr = tl.load(buffer_ptrs + j).to(tl.pointer_type(tl.bfloat16))
                buffer_ptr = tl.multiple_of(buffer_ptr, 16)
                g = tl.load(buffer_ptr + pid * DIM + tl.arange(0, DIM), mask=mask)
                outputs += g
        tl.store(grad_ptr + input_id % V * DIM + tl.arange(0, DIM), outputs, mask=mask)
    ...
    for chunk in range(SIZE):
        ...
        if chunk == RANK:
            if (input_id >= RANK * V) & (input_id < (RANK + 1) * V):
                ...
                buffer_ptr = tl.multiple_of(buffer_ptr, 16)
                w = tl.load(buffer_ptr + pid * D + tl.arange(0, D), mask=mask)
                tl.store(outputs_ptr + pid % M * D + tl.arange(0, D), w, mask=mask)
        else:
            if (input_id >= RANK * V) & (input_id < (RANK + 1) * V):
                w = tl.load(weights_ptr + input_id % V * D + tl.arange(0, D), mask=mask)
                buffer_ptr = tl.load(buffer_ptrs + RANK).to(tl.pointer_type(tl.bfloat16))
...
def triton_sp_embedding_lookup_forward(input_ids, weights, hdl, group):
    ...
    if len(shape) == 2:
        M = shape[0] * shape[1]
        outputs = torch.empty((shape[0], shape[1], d), device=device, dtype=dtype)
        gathered_input_ids = torch.empty((group_size, shape[0], shape[1]), dtype=torch.long, device=device)
    else:
        M = shape[0]
        outputs = torch.empty((M, d), device=device, dtype=dtype)
        gathered_input_ids = torch.empty((group_size, M), dtype=torch.long, device=device)
...
def sp_embedding_lookup_backward_kernel(grad_output_ptr,
    ...
    for chunk in range(SIZE):
        c01 = tl.load(accum_counts_ptr + chunk * (L + 1) + pid + tl.arange(0, 2))
        c0, c1= tl.split(c01)
        if c0 != c1:
            ...
            if chunk == RANK:
                if (input_id >= RANK * V) & (input_id < (RANK + 1) * V):
                    ...
                    tl.atomic_add(grad_ptr + input_id % V * DIM + tl.arange(0, DIM), outputs, mask=mask, sem='relaxed')
                    ...
                    # save to dst addr
                    buffer_ptr = tl.load(buffer_ptrs + input_id // V).to(tl.pointer_type(tl.float32))
                    buffer_ptr = tl.multiple_of(buffer_ptr, 16)
                    tl.store(buffer_ptr + pid * DIM + tl.arange(0, DIM), outputs, mask=mask)
                    symm_mem_sync(
                            signal_ptrs,
                ...
                if (input_id >= RANK * V) & (input_id < (RANK + 1) * V):
                    ...
                    buffer_ptr = tl.multiple_of(buffer_ptr, 16)
                    g = tl.load(buffer_ptr + pid * DIM + tl.arange(0, DIM), mask=mask)
                    tl.atomic_add(grad_ptr + input_id % V * DIM + tl.arange(0, DIM), g, mask=mask, sem='relaxed')
...
def triton_sp_embedding_lookup_backward(grad_output, input_ids, g_ptr, vocab_size, hdl, group, dtype=torch.bfloat16, gathered_input_ids=None):
    ...
    if len(shape) == 2:
        ...
        if gathered_input_ids is None:
            gathered_input_ids = torch.empty((group_size, B, L), dtype=torch.long, device=device)
            dist.all_gather_into_tensor(gathered_input_ids, input_ids, group=group)
    else:
        M = shape[0]
        if gathered_input_ids is None:
    ...
    sorted_ids, sorted_indices = torch.sort(gathered_input_ids.view(group_size, M), stable=False, dim=-1)
    accum_counts = triton_scan_and_count(sorted_ids)
