...
def test_dist_mm(
    M=4096, N=2048, K=4096, coef=1.0, grad_coef=1.0, group=None, bench=False
):
    ...
    local_weights = torch.randn((N, K), dtype=dtype, device=device, requires_grad=False)
    ...
    global_weights = (
        torch.reshape(torch.permute(global_weights, (1, 0, 2)), (N, group_size * K))
        .contiguous()
        .requires_grad_()
    )
    ...
    global_states = torch.reshape(
        torch.permute(global_states, (1, 0, 2)), (M, group_size * K)
    ).contiguous()

    output_ref = global_states @ global_weights.t()

    dist_output = torch_dist_mm(local_states, local_weights, group)
    output_check(output_ref, dist_output, name=f"output:{group_rank}", atol=10.0)

    output = triton_split_tp_gemm(
        local_states,
        local_weights,
        hdl,
        group,
    ...
    if bench:
        ...
        benchmark_func(F.linear, global_states, global_weights, ref_flops=ref_flops)
        benchmark_func(
            torch_dist_mm, local_states, local_weights, group, ref_flops=ref_flops
        )
        benchmark_func(
            triton_split_tp_gemm,
            local_states,
            local_weights,
            hdl,
            group,
