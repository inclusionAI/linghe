...
def test_dist_mm(M=4096, N=2048, K=4096, coef=1.0, grad_coef=1.0, 
    ...
    global_weights = torch.reshape(torch.permute(global_weights, (1,0,2)), (N, group_size * K)).contiguous().requires_grad_()
    ...
    global_states = torch.reshape(torch.permute(global_states, (1,0,2)), (M, group_size * K)).contiguous()
