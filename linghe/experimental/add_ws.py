    ...
    with tlx.async_tasks():
        with tlx.async_task("default"):
            for i in tl.range(loop):
                offsets = block_start + i * BLOCK_SIZE + tl.arange(0,
                                                                   BLOCK_SIZE)
...
def triton_inplace_add_warp_specialized(x: torch.Tensor, y: torch.Tensor):
    ...
    BLOCK_TILE = 64
    grid = lambda meta: (
        triton.cdiv(n_elements, meta["BLOCK_SIZE"] * BLOCK_TILE),)
    inplace_add_warp_specialized_kernel[grid](x, y, n_elements, BLOCK_SIZE=1024,
