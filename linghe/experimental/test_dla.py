...
from linghe.experimental.dla import (
    triton_cp_lightning_attention_forward,
...
def torch_la(q, k, v, decay_scales, s=None, hp=False):
    ...
    att = torch.reshape(att.transpose(1, 2), [B, L, H, D]).contiguous()
...
def rearange(x, group):
    ...
    X = torch.permute(
        torch.reshape(X, (group_size, B, 2, L // 2, H, D)), (1, 2, 0, 3, 4, 5)
    )
    X = torch.reshape(
        torch.cat([X[:, 0], torch.flip(X[:, 1], (1,))], 1), (B, L * group_size, H, D)
    )
...
def test_dist_la(
    ...
    decay_scales = 2 ** (
        -0.5 * torch.arange(1, H + 1, dtype=torch.float32, device=device)
    )
    ...
    output, state = triton_cp_lightning_attention_forward(
        q, k, v, decay_scales, hdl, group, hpc=hpc
    )
    output_check(output_ref, output, atol=-0.2, rtol=0.05, name=f"output:{group_rank}")

    dq, dk, dv = triton_cp_lightning_attention_backward(
        g, q, k, v, state, decay_scales, hdl, group, hpc=hpc
    )
...
if __name__ == "__main__":
    ...
    test_dist_la(
        B=1, L=4096, H=64, D=128, group=group, hpc=True, digest=False, bench=False
    )
