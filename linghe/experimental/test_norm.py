...
from linghe.experimental.norm import triton_rms_norm_forward, triton_parallel_rms_norm_and_block_quant_forward
...
def test_parallel_rmsnorm_and_block_quant(M=4096, N=4096, bench=False):
    ...
    q_ref, scale_ref, rms_ref, qt_ref, scale_t_ref = triton_rms_norm_and_block_quant_forward(x, weight,
                                                                round_scale=False,
                                                                output_mode=2)

    q, scale, rms, qt, scale_t = triton_parallel_rms_norm_and_block_quant_forward(x, weight,
                                                                    round_scale=False,
                                                                    output_mode=2)
    ...
    output_check(scale_t_ref, scale_t, name="parallel.block.t_scale", rtol=-0.125)
    ...
    if bench:
        ...
        benchmark_func(triton_parallel_rms_norm_and_block_quant_forward, x, weight,
                       round_scale=False,
                       output_mode=2,
                       ref_bytes=M * N * 4,
                       n_profile=2)
