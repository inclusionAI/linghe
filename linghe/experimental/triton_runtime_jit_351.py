...
class DependenciesFinder(ast.NodeVisitor):
    ...
    def record_reference(self, val, var_dict=None, name=None):
        ...
        if callable(val) and not isinstance(val, type) and not isinstance(val,
                                                                          constexpr):
...
class JITFunction(JITCallable, KernelInterface[T]):
    ...
    def _pack_args(self, backend, kwargs, bound_args, specialization, options):
        ...
        attrs = {k: backend.parse_attr(get_iterable_path(attrvals, k)) for k in
                 attrs}
    ...
    def run(self, *args, grid, warmup, **kwargs):
        ...
        if kernel is None:
            options, signature, constexprs, attrs = self._pack_args(backend,
                                                                    kwargs,
                                                                    bound_args,
                                                                    specialization,
                                                                    options)
    ...
    def _do_compile(self, key, signature, device, constexprs, options, attrs,
        ...
        if async_mode is not None:
            ...
            def async_compile():
                return self.compile(src, target=target,
