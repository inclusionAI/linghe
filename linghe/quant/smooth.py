    ...
    for i in range(T):
        ...
        if EVEN:
            x = tl.load(x_ptr
                        ...
                        + tl.arange(0, N)[None, :]).to(tl.float32)
        else:
            x = tl.load(x_ptr
                        + pid * W * T * N
                        + i * N * W
                        + tl.arange(0, W)[:, None] * N
                        + tl.arange(0, N)[None, :],
                        mask=indices[:, None] < M).to(tl.float32)
        x *= smooth_scale
...
def triton_smooth_quant(x,
    ...
    if triton.next_power_of_2(N) == N and N <= 8192:
        ...
        EVEN = M % (W * T) == 0
        g = triton.cdiv(M, W * T)
        tokenwise_smooth_quant_kernel[(g,)](x,
                                            x_q,
                                            smooth_scale,
                                            x_scale,
                                            M,
        ...
        T = triton.cdiv(M, W)
        grid = (T,)
        blockwise_smooth_quant_kernel[grid](x,
                                            x_q,
                                            smooth_scale,
                                            x_scale,
                                            M,
...
@triton.jit
def transpose_smooth_quant_kernel(x_ptr,
                                  q_ptr,
                                  ss_ptr,
                                  qs_ptr,
                                  M,
...
@triton.jit
def batch_transpose_smooth_quant_kernel(x_ptr,
                                        q_ptr,
                                        ss_ptr,
                                        qs_ptr,
                                        count_ptr,
    ...
    for i in range(n):
        ...
        tl.store(q_ptr + round_si * N + bid * W * round_count + i * H + tl.arange(0,
                                                                                  W)[
                                                                        :,
                                                                        None] * round_count + tl.arange(0, H)[None, :],
                 xq,
...
def triton_batch_transpose_smooth_quant(x,
                                        smooth_scales,
                                        token_count_per_expert,
                                        splits,
                                        pad=True,
...
@triton.jit
def transpose_rescale_smooth_quant_kernel(x_ptr,
                                          q_ptr,
    ...
    for i in range(m):
        ...
        if EVEN:
            ...
            x = tl.load(x_ptr + offs,
                        mask=(i * H + tl.arange(0, H)[:, None] < M) & (pid * W + tl.arange(0, W)[None, :] < N)).to(
                tl.float32)
            org_quant_scale = tl.load(org_quant_scale_ptr + soffs,
                                      mask=soffs < M, other=0.0)[:, None]
            transpose_smooth_scale = tl.load(transpose_smooth_scale_ptr + soffs,
...
def triton_transpose_rescale_smooth_quant(x_q,
                                          org_smooth_scale,
                                          org_quant_scale,
    ...
    if TAIL:
        ...
        for i in range(T):
            ...
            if REVERSE:
                ...
                smooth_scale = tl.load(ss_ptr + tail_si + i * W + tl.arange(0, W), other=1e30,
                                       mask=mask)
                smooth_scale = 1.0 / smooth_scale
