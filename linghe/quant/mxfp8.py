...
def mxfp8_quant_kernel(x_ptr,
    ...
    if OUTPUT_MODE % 2 == 0:
        ...
        tl.store(x_s_ptr + rid * 32 * b + cid * B // 32 + tl.arange(0, 32)[:, None] * b + tl.arange(0, sb), log_scale+127)
        xq = tl.reshape(xr / scale[:, :, None], (32, B)).to(x_q_ptr.dtype.element_ty)
        tl.store(x_q_ptr + rid * 32 * N + cid * B + \
...
@triton.jit
def batch_mxfp8_quant_kernel(x_ptr,
                                        count_ptr,
                                        xq_ptr,
                                        xs_ptr, 
                                        xtq_ptr,
...
def triton_batch_mxfp8_quant(xs,
