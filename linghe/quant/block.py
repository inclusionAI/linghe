...
def blockwise_quant_kernel(
    ...
    if OUTPUT_MODE % 2 == 0:
        ...
        tl.store(
            x_scale_ptr + rid * 128 + cid * M + tl.arange(0, 128),
            scale,
            mask=indices < M,
        )
    ...
    if OUTPUT_MODE > 0:
        ...
        if ROUND:
            scale = tl.exp2(tl.ceil(tl.log2(scale)))
        tl.store(xt_scale_ptr + rid * N + cid * 128 + tl.arange(0, 128), scale)
...
def triton_blockwise_quant(x, round_scale=False, output_mode=2):
    ...
    x_scale = torch.empty((N // 128, M), device=device, dtype=torch.float32)

    xt_q = torch.empty((N, M), device=device, dtype=torch.float8_e4m3fn)
    xt_scale = torch.empty((triton.cdiv(M, 128), N), device=device, dtype=torch.float32)
...
@triton.jit
def batch_blockwise_quant_kernel(
    x_ptr,
    count_ptr,
    xq_ptr,
    xs_ptr,
    ...
    tl.store(
        xs_ptr + si * nb + cid * count + rid * 128 + tl.arange(0, 128),
        scale,
        mask=rids < count,
    )
    tl.store(
