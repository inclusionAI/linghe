...
@triton.jit
def rms_norm_and_mxfp8_quant_forward_n_kernel(
    x_ptr,
    weight_ptr,
    out_ptr,
    scale_ptr,
    ...
    weight = tl.load(weight_ptr + tl.arange(0, N), mask=weight_mask).to(tl.float32)[
        None, :
    ]
...
@triton.jit
def rms_norm_and_mxfp8_quant_forward_t_kernel(
    x_ptr,
    weight_ptr,
    transpose_output_ptr,
    transpose_scale_ptr,
...
@triton.jit
def rms_norm_and_mxfp8_quant_forward_kernel(
    x_ptr,
    weight_ptr,
    out_ptr,
    scale_ptr,
    ...
    offs = pid * 32 * n + tl.arange(0, 32)[:, None] * n + tl.arange(0, H)[None, :]
    ...
    for i in range(n // H):
        ...
        tl.store(
            transpose_scale_ptr + pid * n + i * H + tl.arange(0, H), log_scale + 127
        )
        x = (x / scale).to(transpose_output_ptr.dtype.element_ty)
        # tl.store(transpose_output_ptr + toffs, tl.trans(x), mask=indices[None, :] < M)
        tl.store(transpose_output_ptr + offs, x)
...
def triton_rms_norm_and_mxfp8_quant_forward(
    x, weight, eps=1e-6, out=None, scale=None, rms=None, output_mode=2
):
    assert x.is_contiguous() and weight.is_contiguous()
    M, n = x.shape
    N = triton.next_power_of_2(n)
    ...
    if output_mode == 0:  # only output non-transpose tensor
        ...
        rms_norm_and_mxfp8_quant_forward_t_kernel[grid](
            x,
            weight,
            transpose_output,
            transpose_scale,
