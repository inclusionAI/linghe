...
def triton_group_rms_norm_gate_forward(
    x: torch.Tensor, gate: torch.Tensor, weight: torch.Tensor, eps=1e-6, group_size=4
):
    """
...
def triton_group_rms_norm_gate_backward(
    grad_output, x, gate, weight, eps=1e-6, group_size=4
):
    length, bs, dim = gate.shape
    assert dim <= 8192 and triton.next_power_of_2(group_size) == group_size
...
def group_rms_norm_gate_and_mxfp8_quant_forward_kernel(
    ...
    x = tl.load(x_ptr + x_offs, mask=row_mask[:, None] & col_mask).to(tl.float32)

    g_offs = out_rows[:, None] * stride_g + gid * d + tl.arange(0, D)[None, :]
    g = tl.load(gate_ptr + g_offs, mask=row_mask[:, None] & col_mask).to(tl.float32)
    ...
    if OUTPUT_MODE % 2 == 0:
        ...
        scale_row = tl.maximum(tl.max(tl.abs(x_blocks), 2) / 448.0, 1e-30)  # [32, SB]
        log_scale_row = tl.ceil(tl.log2(scale_row))
        x_q = tl.reshape(x_blocks / tl.exp2(log_scale_row)[:, :, None], [32, D])

        tl.store(
            x_q_ptr + out_rows[:, None] * DIM + gid * d + tl.arange(0, D)[None, :],
    ...
    if OUTPUT_MODE > 0:
        scale_col = tl.maximum(tl.max(tl.abs(x), 0) / 448.0, 1e-30)
        log_scale_col = tl.ceil(tl.log2(scale_col))
        xt_q = x / tl.exp2(log_scale_col)[None, :]

        tl.store(
            xt_q_ptr + out_rows[:, None] * DIM + gid * d + tl.arange(0, D)[None, :],
            xt_q.to(xt_q_ptr.dtype.element_ty),
            mask=row_mask[:, None] & col_mask,
        )

        tl.store(
            xt_s_ptr + rid * DIM + gid * d + tl.arange(0, D),
...
def triton_group_rms_norm_gate_and_mxfp8_quant_forward(
    ...
    d = dim // group_size
    D = triton.next_power_of_2(d)
    assert D == d, f"d = dim // group_size = {d} only support D==d now"
...
def group_rms_norm_gate_and_mxfp8_quant_backward_kernel(
    ...
    if SHARE:
        ...
        w = tl.load(w_ptr + gid * d + tl.arange(0, D), mask=tl.arange(0, D) < d)
    ...
    for t in tl.static_range(T):
        ...
        go = tl.load(grad_output_ptr + base, mask=row_mask[:, None] & col_mask).to(
            tl.float32
        )
        gv = tl.load(
            gate_ptr
            + out_rows[:, None] * stride_g
            + gid * d
            + tl.arange(0, D)[None, :],
            mask=row_mask[:, None] & col_mask,
        ).to(tl.float32)
        ...
        tl.store(dx_ptr + x_offs, dx.to(tl.bfloat16), mask=row_mask[:, None] & col_mask)
        ...
        if OUTPUT_MODE % 2 == 0:
            ...
            tl.store(
                dg_q_ptr + out_rows[:, None] * DIM + gid * d + tl.arange(0, D)[None, :],
        ...
        if OUTPUT_MODE > 0:
            ...
            tl.store(
                dgt_s_ptr + chunk_rid * DIM + gid * d + tl.arange(0, D),
                (ls_c + 127).to(tl.uint8),
                mask=tl.arange(0, D) < d,
            )

    tmp_row = rid_base * GROUP_SIZE + gid
    tl.store(tmp_dw_ptr + tmp_row * D + tl.arange(0, D), dw, mask=tl.arange(0, D) < d)
...
def triton_group_rms_norm_gate_and_mxfp8_quant_backward(
    ...
    assert d >= 32 and d % 32 == 0, f"d = {d} must be >= 32 and divisible by 32"
    assert (M // 32) % T == 0, f"M//32 = {M // 32} must be divisible by T = {T}"
    assert dim <= 8192 and triton.next_power_of_2(group_size) == group_size
    assert grad_output.is_contiguous()
    assert gate.stride(2) == 1 and gate.stride(0) == gate.stride(1) * bs
    assert length != bs
    ...
    tmp_dw = torch.empty(
        (num_row_blocks * group_size, D), dtype=torch.float32, device=device
    )
