...
def group_rms_norm_gate_forward_kernel(x_ptr, gate_ptr, weight_ptr, out_ptr,
    ...
    if SHARE:
        ...
        weight = tl.load(weight_ptr + tl.arange(0, GROUP_SIZE)[:, None] * d + tl.arange(0,
                                                                                        D
                                                                                        ),
...
def group_rms_norm_gate_backward_kernel(
    ...
    if SHARE:
        ...
        w = tl.load(w_ptr + tl.arange(0, GROUP_SIZE)[:, None] * d + tl.arange(0, D),
                    mask=tl.arange(0, D)[None, :] < d,
                    )

    x_offs = (pid * DIM * T + tl.arange(0, GROUP_SIZE)[:, None] * d + tl.arange(0,
                                                                                D
                                                                                )[
                                                                      None, :]
              )
