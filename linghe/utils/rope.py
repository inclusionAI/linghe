    ...
    for i in range(B):
        if TRANSPOSED:
            ...
            k = tl.load(k_ptr
                        ...
                        + 2 * D * tl.arange(0, h)[:, None]
                        + tl.arange(0, D)[None, :])
        kr = tl.reshape(tl.permute(tl.flip(tl.permute(tl.reshape(k,
                                                                 (h, 2, d)),
                                                      (0, 2, 1)),
                                           dim=2) * signs,
                                   (0, 2, 1)),
        ...
        if TRANSPOSED:
            tl.store(ko_ptr
                     ...
                     + i * h * D * 2
                     + 2 * D * tl.arange(0, h)[:, None]
                     + tl.arange(0, D)[None, :], k)

            k = tl.load(k_ptr
                        + pid * B * k_stride
                        + i * k_stride
                        + D + 2 * D * tl.arange(0, h)[:, None]
                        + tl.arange(0, D)[None, :])
            tl.store(
                ko_ptr
                + pid * B * h * D * 2
    ...
    for i in range(B):
        if TRANSPOSED:
            ...
            q = tl.load(q_ptr
                        ...
                        + 2 * D * tl.arange(0, H)[:, None]
                        + tl.arange(0, D)[None, :]).to(tl.float32)
        qr = tl.reshape(tl.permute(tl.flip(tl.permute(tl.reshape(q,
                                                                 (H, 2, d)),
                                                      (0, 2, 1)),
                                           dim=2) * signs,
                                   (0, 2, 1)),
    ...
    for i in range(B):
        if TRANSPOSED:
            ...
            k = tl.load(k_ptr
                        ...
                        + 2 * D * tl.arange(0, h)[:, None]
                        + tl.arange(0, D)[None, :]).to(tl.float32)
        kr = tl.reshape(tl.permute(tl.flip(tl.permute(tl.reshape(k, (h, 2, d)),
                                                      (0, 2, 1)),
                                           dim=2) * signs,
                                   (0, 2, 1)),
                        (h, D))
...
def triton_half_rope_backward(q_grad,
                              k_grad,
                              freqs,
                              inplace=False,
                              transposed=True):
...
def qk_norm_and_half_rope_forward_kernel(qkv_ptr,
    ...
    for i in range(B):
        if TRANSPOSED:
            q0 = tl.load(q_ptr
                         + pid * B * stride
                         + i * stride
                         + DD * row_offs[:, None] + tl.arange(0, D)[None, :]).to(tl.float32)
            q1 = tl.load(q_ptr
                         + pid * B * stride
                         + i * stride
                         + D
        ...
        tl.store(qo_ptr
                 ...
                 + tl.arange(0, D)[None, :], q1)

        q0 *= rms[:, None]
        q0 *= q_weight_0
        qr = tl.reshape(tl.permute(tl.flip(tl.permute(tl.reshape(q0,
                                                                 (H, 2, d)),
                                                      (0, 2, 1)),
                                           dim=2) * signs,
                                   (0, 2, 1)),
    ...
    for i in range(B):
        ...
        k0 *= rms[:, None]
        k0 *= k_weight_0
        kr = tl.reshape(tl.permute(tl.flip(tl.permute(tl.reshape(k0,
                                                                 (h, 2, d)),
                                                      (0, 2, 1)),
                                           dim=2) * signs,
                                   (0, 2, 1)),
...
def qk_norm_and_half_rope_backward_kernel(gq_ptr, gk_ptr, gv_ptr,
    ...
    for i in range(B):
        ...
        gq_r = tl.reshape(tl.permute(tl.flip(tl.permute(tl.reshape(gq_0,
                                                                   (H, 2, d)),
                                                        (0, 2, 1)),
                                             dim=2) * signs,
                                     (0, 2, 1)),
        ...
        if TRANSPOSED:
            tl.store(dq_ptr
                     ...
                     + i * grad_stride
                     + DD * row_offs[:, None]
                     + tl.arange(0, D)[None, :], dq_0)
            tl.store(dq_ptr
                     + pid * B * grad_stride
                     + i * grad_stride
                     + D + DD * row_offs[:, None]
                     + tl.arange(0, D)[None, :], dq_1)
        else:
            tl.store(dq_ptr
                     + pid * grad_stride
                     + i * L * grad_stride
    ...
    for i in range(B):
        ...
        gk_r = tl.reshape(tl.permute(tl.flip(tl.permute(tl.reshape(gk_0,
                                                                   (h, 2, d)),
                                                        (0, 2, 1)),
                                             dim=2) * signs,
                                     (0, 2, 1)),
        ...
        if TRANSPOSED:
            tl.store(dk_ptr
                     ...
                     + i * grad_stride
                     + DD * row_offs[:, None]
                     + tl.arange(0, D)[None, :], dk_0)
            tl.store(dk_ptr
                     + pid * B * grad_stride
                     + i * grad_stride
                     + D + DD * row_offs[:, None]
                     + tl.arange(0, D)[None, :], dk_1)
        else:
            tl.store(dk_ptr
                     + pid * grad_stride
                     + i * L * grad_stride
    ...
    for i in range(B):
        ...
        if TRANSPOSED:
            tl.store(dv_ptr
                     ...
                     + i * grad_stride
                     + DD * row_offs[:, None]
                     + tl.arange(0, D)[None, :], dv_0)
            tl.store(dv_ptr
                     + pid * B * grad_stride
                     + i * grad_stride
                     + D
                     + DD * row_offs[:, None]
                     + tl.arange(0, D)[None, :], dv_1)
        else:
            tl.store(dv_ptr
                     + pid * grad_stride
                     + i * L * grad_stride
...
@triton.jit
def compatible_qk_norm_and_half_rope_backward_kernel(gq_ptr,
                                                     gk_ptr,
                                                     gv_ptr,
                                                     qkv_ptr,
                                                     q_norm_weight_ptr,
...
def triton_qk_norm_and_half_rope_backward(gq,
                                          gk,
                                          gv,
                                          qkv,
                                          q_norm_weight,
...
def _get_varlen_token_idx(cu_seqlens, pid_m, seq_num, block, cp_rank,
    ...
    if cp_size > 1:
        if token_idx < length // 2:
            ...
            token_idx = (token_idx - length // 2) + (2 * cp_size - cp_rank - 1) * length // 2
    return token_idx
...
# not used
@triton.jit
def _get_fixlen_token_idx(num_tokens, pid_m, seq_num, cp_rank, cp_size,
                          transpose):
...
def varlen_qk_norm_and_half_rope_forward_kernel(qkv_ptr,
    ...
    q0 = tl.load(q_ptr + pid * stride + DD * row_offs[:, None] + tl.arange(
        0, D)[None, :]).to(tl.float32)
    q1 = tl.load(q_ptr + pid * stride + D + DD * row_offs[:, None] + tl.arange(
        0, D)[None, :]).to(tl.float32)
    ...
    if not REUSE:
        pos = _get_varlen_token_idx(cu_seqlens_kv_ptr, pid, B, PB, cp_rank,
                                    CP_SIZE)
        freqs = tl.load(freqs_ptr + pos * D + tl.arange(0, D)).to(tl.float32)
        cos = tl.cos(freqs) * mscale
        sin = tl.sin(freqs) * mscale
    ...
    k0 = tl.load(k_ptr + pid * stride + DD * row_offs[:, None] + tl.arange(
        0, D)[None, :]).to(tl.float32)
    k1 = tl.load(
        k_ptr
        + pid * stride
    ...
    tl.store(
        ...
        + D
        + DD * tl.arange(0, h)[:, None]
        + tl.arange(0, D)[None, :], k1)

    k0 *= rms[:, None]
    k0 *= k_weight_0
    kr = tl.reshape(tl.permute(
        tl.flip(tl.permute(tl.reshape(k0, (h, 2, d)),
                           (0, 2, 1)),
                dim=2) * signs,
        (0, 2, 1)),
        (h, D))
...
def compatible_varlen_qk_norm_and_half_rope_forward_kernel(qkv_ptr,
    ...
    v0 = tl.load(v_ptr + pid * stride + DD * row_offs[:, None] + tl.arange(
        0, D)[None, :], mask=row_mask).to(tl.float32)
    v1 = tl.load(
        v_ptr
        + pid * stride
...
def triton_varlen_qk_norm_and_half_rope_forward(qkv, q_norm_weight,
                                                k_norm_weight,
                                                freqs,
...
def varlen_qk_norm_and_half_rope_backward_kernel(gq_ptr, gk_ptr, gv_ptr,
    ...
    gq_r = tl.reshape(tl.permute(
        tl.flip(tl.permute(tl.reshape(gq_0,
                                      (H, 2, d)),
                           (0, 2, 1)),
                dim=2) * signs,
        (0, 2, 1)),
    ...
    if SILU:
        ...
        r = tl.rsqrt((tl.sum(q_0 * q_0, 1)
                      + tl.sum(q_1 * q_1, 1)) / DD + eps)[:, None]
    ...
    gk_r = tl.reshape(tl.permute(
        tl.flip(tl.permute(tl.reshape(gk_0,
                                      (h, 2, d)),
                           (0, 2, 1)),
                dim=2) * signs,
        (0, 2, 1)),
    ...
    tl.store(dk_ptr + pid * grad_stride + DD * row_offs[:, None] + tl.arange(
        0, D)[None, :], dk_0)
    tl.store(dk_ptr + pid * grad_stride + D + DD * row_offs[:, None] + tl.arange(
        0, D)[None, :], dk_1)
...
@triton.jit
def compatible_varlen_qk_norm_and_half_rope_backward_kernel(gq_ptr, gk_ptr,
                                                            gv_ptr,
                                                            qkv_ptr,
                                                            q_norm_weight_ptr,
                                                            k_norm_weight_ptr,
    ...
    if SILU:
        ...
        r = tl.rsqrt((tl.sum(k_0 * k_0, 1)
                      + tl.sum(k_1 * k_1, 1)) / DD + eps)[:, None]
    ...
    tl.store(dk_ptr
             + pid * grad_stride
             + DD * row_offs[:, None]
             + tl.arange(0, D)[None, :], dk_0, mask=row_mask)
    tl.store(dk_ptr
             + pid * grad_stride
             + D
             + DD * row_offs[:, None]
             + tl.arange(0, D)[None, :], dk_1, mask=row_mask)
...
def triton_varlen_qk_norm_and_half_rope_backward(gq,
                                                 gk,
                                                 gv,
                                                 qkv,
                                                 q_norm_weight,
...
def mla_rope_forward_kernel(q_ptr, kv_ptr, k_pos_emb_ptr,
    ...
    if VARLEN:
        pos = _get_varlen_token_idx(cu_seqlens_q_ptr, pid, B, PB, cp_rank,
                                    cp_size)
        L = num_tokens
        bid = 0
    else:
...
def triton_mla_rope_forward(q,
    ...
    assert kv.stride(-2) == 256 and k_pos_emb.stride(
        -2) == 64, f"{kv.stride()=} {k_pos_emb.stride()=}"
    num_stages = 2
    num_warps = 2
...
def mla_rope_backward_kernel(q_ptr, k_ptr, v_ptr, freqs_ptr,
    ...
    q0 = tl.load(q_ptr
                 ...
                 + 128
                 + 192 * tl.arange(0, H)[:, None]
                 + tl.arange(0, 32)[None, :]).to(tl.float32)
    q1 = tl.load(q_ptr
                 + pid * H * 192
                 + 160
                 + 192 * tl.arange(0, H)[:, None]
...
def triton_mla_rope_backward(q_grad, k_grad, v_grad, freqs, mscale=1.0,
                             transposed=False,
                             cu_seqlens_q=None, cu_seqlens_kv=None, cp_rank=0,
                             cp_size=1,
                             reuse=False):
