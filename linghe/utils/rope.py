...
def compatible_qk_norm_and_half_rop_forward_kernel(qkv_ptr,
    ...
    for i in range(B):
        if TRANSPOSED:
            q0 = tl.load(q_ptr + pid * B * stride + i * stride + DD * row_offs[:, None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
            q1 = tl.load(q_ptr + pid * B * stride + i * stride + D + DD * row_offs[:, None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
        else:
            q0 = tl.load(q_ptr + i * L * stride + pid * stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
            q1 = tl.load(q_ptr + i * L * stride + pid * stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
        if SILU:
        ...
        tl.store(qo_ptr + pid * H * DD + i * L * H * DD + D + DD * tl.arange(0, H_p)[:,None] + tl.arange(0, D)[None, :], q1, mask=q_mask)
        ...
        q0 = q0 * cos + qr * sin
        tl.store(qo_ptr + pid * H * DD + i * L * H * DD + DD * tl.arange(0, H_p)[:,None] + tl.arange(0,D)[None, :], q0, mask=q_mask)
    ...
    for i in range(B):
        if TRANSPOSED:
            k0 = tl.load(k_ptr + pid * B * stride + i * stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
            k1 = tl.load(k_ptr + pid * B * stride + i * stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
        else:
            k0 = tl.load(k_ptr + i * L * stride + pid * stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
            k1 = tl.load(k_ptr + i * L * stride + pid * stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
        ...
        k0 = k0 * cos + kr * sin
        tl.store(
            ko_ptr + pid * h * DD + i * L * h * DD + DD * tl.arange(0, h_p)[:,None] + tl.arange(0,D)[None, :], k0,
            mask=k_mask
            )
    ...
    for i in range(B):
        if TRANSPOSED:
            v0 = tl.load(v_ptr + pid * B * stride + i * stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
            v1 = tl.load(v_ptr + pid * B * stride + i * stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
        else:
            v0 = tl.load(v_ptr + i * L * stride + pid * stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
            v1 = tl.load(v_ptr + i * L * stride + pid * stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
        if SILU:
            v0 = v0 * tl.sigmoid(v0)
            v1 = v1 * tl.sigmoid(v1)
        ...
        tl.store(
            vo_ptr + pid * h * DD + i * L * h * DD + DD * tl.arange(0, h_p)[:,None] + tl.arange(0,D)[None, :], v0, mask=v_mask)
        tl.store(
            vo_ptr + pid * h * DD + i * L * h * DD + D + DD * tl.arange(0, h_p)[:,None] + tl.arange(0, D)[None, :], v1, mask=v_mask)
...
def compatible_qk_norm_and_half_rope_backward_kernel(gq_ptr, gk_ptr, gv_ptr,
    ...
    for i in range(B):
        gq_0 = tl.load(
                gq_ptr + i * L * H * DD + pid * H * DD + DD * tl.arange(0, H_p)[:,None] + tl.arange(0,D)[None, :]
                , mask=tl.arange(0, H_p)[:,None]< H
            ).to(tl.float32)
        gq_1 = tl.load(
            gq_ptr + i * L * H * DD + pid * H * DD + D + DD * tl.arange(0, H_p)[:,None] + tl.arange(0, D)[None, :]
                , mask=tl.arange(0, H_p)[:,None]< H
            ).to(tl.float32)
        ...
        if TRANSPOSED:
            ...
            q0 = tl.load(q_ptr + pid * stride + i * L * stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
            q1 = tl.load(q_ptr + pid * stride + i * L * stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
        ...
        if TRANSPOSED:
            ...
            tl.store(dq_ptr + pid * B * grad_stride + i * grad_stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dq_0, mask=row_mask)
            tl.store(dq_ptr + pid * B * grad_stride + i * grad_stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dq_1, mask=row_mask)
            ...
            tl.store(dq_ptr + pid * grad_stride + i * L * grad_stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dq_0, mask=row_mask)
            tl.store(dq_ptr + pid * grad_stride + i * L * grad_stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dq_1, mask=row_mask)
    ...
    for i in range(B):
        gk_0 = tl.load(
            gk_ptr + i * L * h * DD + pid * h * DD + DD * tl.arange(0, h_p)[:,None] + tl.arange(0,D)[None, :],
            mask=tl.arange(0, h_p)[:,None] < h
            ).to(tl.float32)
        gk_1 = tl.load(
            gk_ptr + i * L * h * DD + pid * h * DD + D + DD * tl.arange(0, h_p)[:,None] + tl.arange(0, D)[None, :],
            mask=tl.arange(0, h_p)[:,None] < h
            ).to(tl.float32)
        ...
        if TRANSPOSED:
            ...
            k0 = tl.load(k_ptr + pid * stride + i * L * stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
            k1 = tl.load(k_ptr + pid * stride + i * L * stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], mask=row_mask).to(tl.float32)
        ...
        if TRANSPOSED:
            tl.store(dk_ptr + pid * B * grad_stride + i * grad_stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dk_0, mask=row_mask)
            tl.store(dk_ptr + pid * B * grad_stride + i * grad_stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dk_1, mask=row_mask)
        else:
            tl.store(dk_ptr + pid * grad_stride + i * L * grad_stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dk_0, mask=row_mask)
            tl.store(dk_ptr + pid * grad_stride + i * L * grad_stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dk_1, mask=row_mask)
    ...
    for i in range(B):
        ...
        gv_0 = tl.load(
            gv_ptr + i * L * h * DD + pid * h * DD + DD * tl.arange(0, h_p)[:,None] + tl.arange(0,D)[ None, :], mask=tl.arange(0, h_p)[:,None] < h).to(tl.float32)
        gv_1 = tl.load(
            gv_ptr + i * L * h * DD + pid * h * DD + D + DD * tl.arange(0, h_p)[:,None] + tl.arange(0, D)[None, :], mask=tl.arange(0, h_p)[:,None] < h).to(tl.float32)
        ...
        if TRANSPOSED:                                        
            tl.store(dv_ptr + pid * B * grad_stride + i * grad_stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dv_0, mask=row_mask)
            tl.store(dv_ptr + pid * B * grad_stride + i * grad_stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dv_1, mask=row_mask)
        else:
            tl.store(dv_ptr + pid * grad_stride + i * L * grad_stride + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dv_0, mask=row_mask)   
            tl.store(dv_ptr + pid * grad_stride + i * L * grad_stride + D + DD * row_offs[:,None] + tl.arange(0, D)[None, :], dv_1, mask=row_mask)
...
@triton.jit
def compatible_varlen_qk_norm_and_half_rope_forward_kernel(qkv_ptr,
                                            q_norm_weight_ptr, k_norm_weight_ptr,
                                            freqs_ptr,
                                            cu_seqlens_q_ptr,
                                            cu_seqlens_kv_ptr,
    ...
    q0 *= q_weight_0
    qr = tl.reshape(tl.permute(
        tl.flip(tl.permute(tl.reshape(q0, (PH, 2, d)), (0, 2, 1)),
                dim=2) * signs, (0, 2, 1)), (PH, D))
    q0 = q0 * cos + qr * sin
    tl.store(
        qo_ptr + pid * H * DD + DD * tl.arange(0, PH)[:,
                                                        None] + tl.arange(0,
                                                                        D)[
                                                                None, :], q0,
        mask=q_mask)
    ...
    tl.store(
        ko_ptr + pid * h * DD + D + DD * tl.arange(0, ph)[:,
                                                            None] + tl.arange(
            0, D)[None, :], k1, mask=k_mask)

    k0 *= rms[:, None]
    k0 *= k_weight_0
    kr = tl.reshape(tl.permute(
        tl.flip(tl.permute(tl.reshape(k0, (ph, 2, d)), (0, 2, 1)),
                dim=2) * signs, (0, 2, 1)), (ph, D))
    k0 = k0 * cos + kr * sin
    tl.store(
        ko_ptr + pid * h * DD + DD * tl.arange(0, ph)[:,
                                                        None] + tl.arange(0,
                                                                        D)[
                                                                None, :], k0,
        mask=k_mask)
    ...
    tl.store(
        vo_ptr + pid * h * DD + DD * tl.arange(0, ph)[:,
                                                    None] + tl.arange(0,
                                                                        D)[
                                                            None, :], v0, mask=v_mask)
    tl.store(
...
def triton_varlen_qk_norm_and_half_rope_forward(qkv, q_norm_weight, k_norm_weight,
                                         freqs, 
...
@triton.jit
def varlen_qk_norm_and_half_rope_backward_kernel(gq_ptr, gk_ptr, gv_ptr,
                                          qkv_ptr,
                                          q_norm_weight_ptr, k_norm_weight_ptr,
                                          freqs_ptr,
                                          cu_seqlens_q_ptr,
    ...
    gq_0 = tl.load(
        gq_ptr + pid * H * DD + DD * tl.arange(0, H)[:,
                                                        None] + tl.arange(0,
                                                                        D)[
                                                                None, :]).to(tl.float32)
    gq_1 = tl.load(
    ...
    tl.store(dq_ptr + pid * grad_stride + DD * row_offs[:,
                                                        None] + tl.arange(
        0, D)[None, :], dq_0)
    tl.store(dq_ptr + pid * grad_stride + D + DD * row_offs[:,
                                                            None] + tl.arange(
        0, D)[None, :], dq_1)
    ...
    gk_0 = tl.load(
        gk_ptr + pid * h * DD + DD * tl.arange(0, h)[:,
                                                        None] + tl.arange(0,
                                                                        D)[
                                                                None, :]).to(tl.float32)
    gk_1 = tl.load(
    ...
    tl.store(dk_ptr + pid * grad_stride + DD * row_offs[:,
                                                        None] + tl.arange(
        0, D)[None, :], dk_0)
    tl.store(dk_ptr + pid * grad_stride + D + DD * row_offs[:,
                                                            None] + tl.arange(
        0, D)[None, :], dk_1)
    ...
    gv_0 = tl.load(
        gv_ptr + pid * h * DD + DD * tl.arange(0, h)[:,
                                                    None] + tl.arange(0,
                                                                        D)[
                                                            None, :]).to(tl.float32)
    gv_1 = tl.load(
    ...
    tl.store(dv_ptr + pid * grad_stride + DD * row_offs[:,
                                                        None] + tl.arange(
        0, D)[None, :], dv_0)
    tl.store(dv_ptr + pid * grad_stride + D + DD * row_offs[:,
                                                            None] + tl.arange(
        0, D)[None, :], dv_1)
...
@triton.jit
def compatible_varlen_qk_norm_and_half_rope_backward_kernel(gq_ptr, gk_ptr, gv_ptr,
                                          qkv_ptr,
                                          q_norm_weight_ptr, k_norm_weight_ptr,
                                          freqs_ptr,
                                          cu_seqlens_q_ptr,
    ...
    gq_0 = tl.load(
        gq_ptr + pid * H * DD + DD * tl.arange(0, PH)[:,
                                                        None] + tl.arange(0,
                                                                        D)[
                                                                None, :],
        mask=tl.arange(0, PH)[:,None]< H).to(tl.float32)
    gq_1 = tl.load(
    ...
    gq_r = tl.reshape(tl.permute(
        tl.flip(tl.permute(tl.reshape(gq_0, (PH, 2, d)), (0, 2, 1)),
                dim=2) * signs, (0, 2, 1)), (PH, D))
    gq_0 = gq_0 * cos + gq_r * sin
    ...
    gk_0 = tl.load(
        gk_ptr + pid * h * DD + DD * tl.arange(0, ph)[:,
                                                        None] + tl.arange(0,
                                                                        D)[
                                                                None, :],
        mask=tl.arange(0, ph)[:,None] < h).to(tl.float32)
    gk_1 = tl.load(
        gk_ptr + pid * h * DD + D + DD * tl.arange(0, ph)[:,
                                                            None] + tl.arange(
            0, D)[None, :],mask=tl.arange(0, ph)[:,None] < h).to(tl.float32)

    gk_r = tl.reshape(tl.permute(
        tl.flip(tl.permute(tl.reshape(gk_0, (ph, 2, d)), (0, 2, 1)),
                dim=2) * signs, (0, 2, 1)), (ph, D))
    gk_0 = gk_0 * cos + gk_r * sin
    ...
    gv_0 = tl.load(
        gv_ptr + pid * h * DD + DD * tl.arange(0, ph)[:,
                                                    None] + tl.arange(0,
                                                                        D)[
                                                            None, :],
        mask=tl.arange(0, ph)[:,None] < h).to(tl.float32)
...
def triton_varlen_qk_norm_and_half_rope_backward(gq, gk, gv, qkv, q_norm_weight,
                                          k_norm_weight, freqs, 
                                          cu_seqlens_q, cu_seqlens_kv, 
                                          eps=1e-6,
                                          interleaved=True,
...
def mla_rope_forward_kernel(q_ptr, kv_ptr, k_pos_emb_ptr, 
    ...
    q = tl.load(
        q_ptr + pid * H * 192 + 128 + 192 * tl.arange(0, H)[:, None] + tl.arange(0, 64)[None, :]).to(tl.float32)
    ...
    if TRANSPOSE:
        # [L, B, H, D] -> [B, L, H, D]
        qn = tl.load(q_ptr + pid * H * 192 + 192 * tl.arange(0, H)[:, None] + tl.arange(0, 128)[None, :]).to(tl.float32)
        tl.store(
    ...
    k = tl.load(
        kv_ptr + pid * H * 256 + 256 * tl.arange(0, H)[: ,None] + tl.arange(0, 128)[None, :]).to(tl.float32)
    if TRANSPOSE:
        tl.store(
    ...
    v = tl.load(
        kv_ptr + pid * H * 256 + 128 + 256 * tl.arange(0, H)[:, None] + tl.arange(0, 128)[None, :]).to(tl.float32)
    if TRANSPOSE:
        tl.store(
