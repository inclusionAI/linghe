...
def weighted_silu_forward_asm_kernel(
    ...
    if WEIGHT:
        w = tl.load(weight_ptr
                    + indices,
                    mask=indices < M).to(tl.float32)[:, None]
...
def silu_and_block_quant_forward_kernel(x_ptr,
    ...
    if OUTPUT_MODE % 2 == 0:
        ...
        x2 = tl.load(x_ptr + n + offs, mask=mask).to(tl.float32)
        if CUTOFF:
            x = tl.minimum(x1 * tl.sigmoid(x1), limit) * tl.clamp(x2, -limit, limit)
        else:
            x = x1 * tl.sigmoid(x1) * x2
    ...
    if OUTPUT_MODE > 0:
        # reload is faster than transpose
        x1 = tl.load(x_ptr + offs, mask=mask).to(tl.float32)
        x2 = tl.load(x_ptr + n + offs, mask=mask).to(tl.float32)
        if CUTOFF:
            x = tl.minimum(x1 * tl.sigmoid(x1), limit) * tl.clamp(x2, -limit, limit)
...
def triton_silu_and_block_quant_forward(x,
    ...
    transpose_scale = torch.empty((triton.cdiv(M, 128), n), device=device,
                                  dtype=torch.float32)
    if output_mode == 0:
        H, W, num_warps = 64, 128, 8
    elif output_mode == 1:
        H, W, num_warps = 128, 64, 8
    else:
...
def silu_and_block_quant_backward_kernel(g_ptr, x_ptr,
    ...
    x2 = tl.load(x_ptr + n + offs, mask=mask).to(tl.float32)
    if CUTOFF:
        dx2_mask = (x2 <= limit) & (x2 >= -limit)
        x2 = tl.clamp(x2, -limit, limit)
    g = tl.load(g_ptr
                + rid * 128 * n
                + cid * 128
    ...
    if CUTOFF:
        dx1_mask = (sigmoid * x1 <= limit).to(tl.float32)
        dx1 *= dx1_mask
    scale1 = tl.maximum(
    ...
    if CUTOFF:
        dx2 = g * tl.minimum(sigmoid * x1, limit) * dx2_mask.to(tl.float32)
    else:
        dx2 = g * sigmoid * x1
    scale2 = tl.maximum(
...
def batch_weighted_silu_and_block_quant_forward_kernel(x_ptr, weight_ptr,
    ...
    if CUTOFF:
        x = tl.minimum(x1 * tl.sigmoid(x1), limit) * tl.clamp(x2, -limit, limit) * w[:, None]
    else:
        x = x1 * tl.sigmoid(x1) * x2 * w[:, None]

    scale1 = tl.maximum(tl.max(tl.abs(x), 1) / 448, 1e-30)
    scale2 = tl.maximum(tl.max(tl.abs(x), 0) / 448, 1e-30)
    if ROUND:
        scale1 = tl.exp2(tl.ceil(tl.log2(scale1)))
        scale2 = tl.exp2(tl.ceil(tl.log2(scale2)))
    ...
    tl.store(
        ...
        scale1,
        mask=indices < count)
    xq1 = (x / scale1[:, None]).to(out_ptr.dtype.element_ty)
    tl.store(out_ptr + hoffs, xq1, mask=mask)
    ...
    toffs = (si * n
             ...
             + tl.arange(0, 128)[None, :])
    xq2 = tl.trans((x / scale2).to(out_ptr.dtype.element_ty))
    tl.store(transpose_output_ptr + toffs, xq2,
             mask=indices[None, :] < count)
...
def batch_weighted_silu_and_block_quant_forward_nt_kernel(x_ptr, weight_ptr,
    ...
    maxs = tl.zeros((B, 128), dtype=tl.float32)
    for i in range(I):
        mask = indices[:, None] < count
    ...
    hoffs = (si * n
             ...
             + tl.arange(0, 128)[None, :])
    indices = rid * 128 + tl.arange(0, B)
    scale = tl.maximum(tl.max(maxs, 0) / 448, 1e-30)
    if ROUND:
        scale = tl.exp2(tl.ceil(tl.log2(scale)))
    ...
    n_blocks = tl.cdiv(counts, 128)
    transpose_scale_off = tl.sum(tl.where(tl.arange(0, E) < eid, n_blocks, 0))
    tl.store(transpose_scale_ptr + transpose_scale_off * n + rid * n + cid * 128 + tl.arange(0, 128),
                scale)
    toffs = (si * n
             + rid * 128
             + cid * count * 128
...
def batch_weighted_silu_and_block_quant_forward_t_kernel(x_ptr, weight_ptr,
    ...
    if ROUND:
        scale = tl.exp2(tl.ceil(tl.log2(scale)))
    tl.store(
        transpose_scale_ptr + transpose_scale_off * n + rid * n + cid * B + tl.arange(0, B), scale)
    ...
    toffs = (si * n
             ...
             + tl.arange(0, 128)[None, :])
    tl.store(transpose_output_ptr + toffs,
             xq,
             mask=indices[None, :] < count)
...
def triton_batch_weighted_silu_and_block_quant_forward(x,
    ...
    if output_mode == 0:
        ...
        batch_weighted_silu_and_block_quant_forward_nt_kernel[grid](x,
                                                                 weight,
                                                                 out,
                                                                 scale,
                                                                 transpose_output,
...
def batch_weighted_silu_and_block_quant_backward_kernel(g_ptr, x_ptr,
    ...
    tl.store(dx_ptr + offs, dx / scale1[:, None], mask=idx[:, None] < count)
    ...
    tl.store(
        ...
        + rid * n * 2
        + cid * 128
        + tl.arange(0, 128), scale2)

    qdx = tl.trans((dx / scale2[None, :]).to(dx_ptr.dtype.element_ty))
    ...
    if ROUND:
        scale3 = tl.exp2(tl.ceil(tl.log2(scale3)))
    tl.store(
        dx_scale_ptr
        + si * nb * 2
        + cid * count
        ...
        scale3,
        mask=idx < count)
    tl.store(dx_ptr + n + offs, dx / scale3[:, None], mask=idx[:, None] < count)


    scale4 = tl.maximum(
        tl.max(dx.abs(), 0) / 448, 1e-30)
    if ROUND:
        scale4 = tl.exp2(tl.ceil(tl.log2(scale4)))
    qdx = tl.trans((dx / scale4[None, :]).to(dx_ptr.dtype.element_ty))
    tl.store(
        transpose_dx_scale_ptr
        + transpose_off * n * 2
        + rid * n * 2 + n
        + cid * 128
        + tl.arange(0, 128), scale4)
    tl.store(
        transpose_dx_ptr
        + count * n
        + si * n * 2
...
def silu_and_mxfp8_quant_backward_kernel(g_ptr,
    ...
    g = tl.load(g_ptr + rid * 32 * n + cid * 32 +
                tl.arange(0, 32)[:, None] * n +
                tl.arange(0, 32)[None, :], mask=mask)  # .to(tl.float32)
...
# used in shared expert
def triton_silu_and_mxfp8_quant_backward(g, x, limit=None):
    """
    backward of triton_silu_and_mxfp8_quant_forward
    Args:
        g: gradient
        x: input tensor
...
def silu_and_smooth_quant_backward_kernel(g_ptr,
    ...
    transpose_smooth_scale = tl.load(
        transpose_smooth_scale_ptr + pid * T + tl.arange(0, T))[:, None]
    for i in range(nb):
...
def triton_silu_and_smooth_quant_backward(g, x,
    ...
    grid = (N // B, M // T)
    _requant_kernel[grid](transpose_dx,
                          transpose_dx_scale,
                          transpose_dx_scales,
                          N,
                          M,
...
def batch_weighted_silu_and_smooth_quant_forward_kernel(x_ptr,
    ...
    for i in range(c):
        ...
        w = tl.load(weight_ptr
                    + si
                    + indices,
                    mask=indices < count).to(tl.float32)[:, None]
        x = x1 * tl.sigmoid(x1) * x2
...
def _batch_requant_kernel(x_ptr,
    ...
    counts = tl.load(count_ptr + tl.arange(0, E))
    round_off = tl.sum(tl.where(tl.arange(0, E) < eid,
                                tl.cdiv(counts, 32) * 32,
                                0))

    offs = (round_off * N
            + rid * H * round_count
            + cid * W + tl.arange(0, H)[:, None] * round_count
            + tl.arange(0, W)[None, :])
...
def triton_batch_weighted_silu_and_smooth_quant_backward(g, x, weight,
    ...
    if s == 0:
        transpose_dx_scale = torch.zeros((n_expert, N),
                                         device=device,
                                         dtype=torch.float32)
        return dx, dx_scale, dw, transpose_dx, transpose_dx_scale
    else:
    ...
    _batch_requant_kernel[grid](transpose_dx,
                                transpose_dx_scale,
                                transpose_dx_scales,
                                counts,
                                N,
