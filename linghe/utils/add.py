...
def inplace_add_kernel(
    x_ptr, y_ptr, N, B: tl.constexpr, EVEN: tl.constexpr, ACCUM: tl.constexpr
):
...
def triton_inplace_add(x: torch.Tensor, y: torch.Tensor, accum: bool = True):
    ...
    inplace_add_kernel[grid](
        x, y, N, B, EVEN, accum, num_stages=num_stages, num_warps=num_warps
    )
    return x


@triton.jit
def batch_inplace_add_kernel(
    x_ptrs,
    y_ptrs,
    size_ptr,
    T,
...
def triton_batch_inplace_add(
    xs: List[torch.Tensor], ys: List[torch.Tensor], accum: bool = True
):
    """
    inplace add y to x
    Args:
    ...
    x_ptrs = torch.tensor([x.data_ptr() for x in xs], dtype=torch.int64).cuda(
        device, non_blocking=True
    )
    y_ptrs = torch.tensor([y.data_ptr() for y in ys], dtype=torch.int64).cuda(
        device, non_blocking=True
    )
    x_dtype = xs[0].dtype
