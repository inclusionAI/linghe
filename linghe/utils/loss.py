    ...
    if label == ignore_index:
        for i in range(T):
            ...
            if INPLACE:
                ...
                tl.store(
                    input_grad_ptr + pid * N + i * B + tl.arange(0, B),
                    grad,
                    mask=i * B + tl.arange(0, B) < N,
                )
        return
...
def triton_softmax_cross_entropy_backward(
    logits, labels, sum_exp, max_logit, output_grad, ignore_index=-100, inplace=False
):
    """
...
@triton.jit
def parallel_logit_stat_kernel(
    logit_ptr,
    label_ptr,
    sum_exp_ptr,
    max_logit_ptr,
...
def triton_parallel_softmax_cross_entropy_forward(
    logits, labels, group, ignore_index=-100
):
    """
    compute token-wise softmax cross entropy loss
    Args:
    ...
    parallel_calc_loss_kernel[grid](
        labels,
        statistic,
        sum_exp,
        max_logit,
...
@triton.jit
def parallel_softmax_cross_entropy_backward_kernel(
    logit_ptr,
    label_ptr,
    sum_exp_ptr,
    max_logit_ptr,
...
@triton.jit
def moe_z_loss_forward_kernel(
    logit_ptr, loss_ptr, coef, T: tl.constexpr, D: tl.constexpr
):
    pid = tl.program_id(axis=0)

    logit = tl.load(
        logit_ptr + pid * T * D + tl.arange(0, T)[:, None] * D + tl.arange(0, D)
    ).to(tl.float32)
...
def triton_moe_z_loss_forward(logits, coef=1e-6):
    ...
    grid = (M // T,)
    moe_z_loss_forward_kernel[grid](logits, loss, coef, T, D, num_stages=3, num_warps=1)
    return loss.mean()


@triton.jit
def moe_z_loss_backward_kernel(
    input_grad_ptr, logit_ptr, output_grad_ptr, coef, T: tl.constexpr, D: tl.constexpr
):
    pid = tl.program_id(axis=0)
    ...
    tl.store(
        output_grad_ptr + pid * T * D + tl.arange(0, T)[:, None] * D + tl.arange(0, D),
        grads,
    )
...
def triton_moe_z_loss_backward(grads, logits, coef=1e-6):
    ...
    moe_z_loss_backward_kernel[grid](
        grads, logits, output_grad, coef, T, D, num_stages=3, num_warps=1
    )
    return output_grad
