...
def triton_mxfp8_gemm(a: torch.Tensor,
                      b: torch.Tensor,
                      a_s: torch.Tensor,
                      b_s: torch.Tensor,
...
def triton_mxfp8_gemm_forward(a: torch.Tensor,
                      b: torch.Tensor,
                      a_s: torch.Tensor,
                      b_s: torch.Tensor,
                      out: Optional[torch.tensor] = None,
...
def triton_mxfp8_gemm_backward(a: torch.Tensor,
                      b: torch.Tensor,
                      a_s: torch.Tensor,
                      b_s: torch.Tensor,
                      out: Optional[torch.tensor] = None,
...
def triton_mxfp8_gemm_update(a: torch.Tensor,
                      b: torch.Tensor,
                      a_s: torch.Tensor,
                      b_s: torch.Tensor,
                      out: Optional[torch.tensor] = None,
...
def triton_mxfp8_grouped_gemm(a: List[torch.Tensor],
                              b: List[torch.Tensor],
    ...
    grid = (len(m_splits),
            M // BLOCK_SIZE_M,
            N // BLOCK_SIZE_N)  # noqa
    mxfp8_grouped_gemm_kernel[grid](a_ptrs, b_ptrs,
                                    out_ptrs if layout == 'NT' else out,
                                    as_ptrs, bs_ptrs, sizes, accums,
                                    M, N, K,
                                    BLOCK_SIZE_M=BLOCK_SIZE_M,
                                    BLOCK_SIZE_N=BLOCK_SIZE_N,
                                    ACCUM=accumulate,
...
def triton_mxfp8_grouped_gemm_forward(a: List[torch.Tensor],
                              b: List[torch.Tensor],
                              a_s: List[torch.Tensor],
                              b_s: List[torch.Tensor],
                              m_splits: List[int],
    ...
    grid = (len(m_splits),
            max(m_splits) // BLOCK_SIZE_M,
            N // BLOCK_SIZE_N)  # noqa
    mxfp8_grouped_gemm_forward_kernel[grid](a_ptrs, b_ptrs,
                                    out,
                                    as_ptrs, bs_ptrs, sizes, accums,
                                    N, K,
                                    BLOCK_SIZE_M=BLOCK_SIZE_M,
...
def triton_mxfp8_grouped_gemm_backward(a: List[torch.Tensor],
    ...
    grid = (len(m_splits),
            max(m_splits) // BLOCK_SIZE_M,
            N // BLOCK_SIZE_N)  # noqa
    mxfp8_grouped_gemm_backward_kernel[grid](a_ptrs, b_ptrs,
                                    out,
                                    as_ptrs, bs_ptrs, sizes, accums,
                                    N, K,
                                    BLOCK_SIZE_M=BLOCK_SIZE_M,
...
def triton_mxfp8_grouped_gemm_update(a: List[torch.Tensor],
                              b: List[torch.Tensor],
                              a_s: List[torch.Tensor],
                              b_s: List[torch.Tensor],
                              m_splits: List[int],
    ...
    grid = (len(m_splits),
            M // BLOCK_SIZE_M,
            N // BLOCK_SIZE_N)  # noqa
    mxfp8_grouped_gemm_update_kernel[grid](a_ptrs, b_ptrs,
                                    out_ptrs,
                                    as_ptrs, bs_ptrs, sizes, accums,
                                    M, N,
                                    BLOCK_SIZE_M=BLOCK_SIZE_M,
...
def triton_native_mxfp8_gemm(a: torch.Tensor,
                      b: torch.Tensor,
                      a_s: torch.Tensor,
                      b_s: torch.Tensor,
                      out: Optional[torch.tensor] = None,
