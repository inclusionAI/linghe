...
from linghe.utils.scatter import (
    ...
    triton_unpermute_with_reverse_map,
)
...
class _MXFP8PermuteDP(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        tokens,
...
def mxfp8_permute(
    ...
    permuted_input, permuted_probs, row_id_map, row_id_index = _MXFP8PermuteDP.apply(
        tokens,
        probs,
        routing_map,
        tokens_per_expert_cuda_tensor,
...
class _MXFP8UnpermuteDP(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        permuted_tokens,
...
def mxfp8_unpermute(
    ...
    output = _MXFP8UnpermuteDP.apply(
        permuted_tokens,
        row_id_map,
        row_id_index,
        tokens_per_expert,
...
class _SmoothPermute(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, tokens, probs, routing_map, tokens_per_expert, splits, quantizers, cls
    ):
        """Forward function."""
        num_tokens, hidden_dim = tokens.shape
    ...
    def backward(ctx, grad_output, grad_prob, grad_map, grad_indices):
        """Backward function."""
        output, prob_output = triton_unpermute_with_mask_map(
            grad_output, ctx.row_id_map, grad_prob
        )
        return (
            output.view(ctx.shape),
            prob_output.view(ctx.prob_shape),
...
def smooth_permute(
    ...
    Args:
        ...
        drop_and_pad (bool, optional): Whether or not the token dispatcher uses token-drop
                                       ...
                                       in each column.
    """
    (permuted_input, permuted_probs, row_id_map, row_id_indices) = _SmoothPermute.apply(
        tokens, probs, routing_map, tokens_per_expert, splits, quantizers, cls
    )
    return permuted_input, permuted_probs, row_id_map, row_id_indices
...
def smooth_fused_permute(
    ...
    (permuted_input, permuted_probs, row_id_map, row_id_indices) = (
        _SmoothFusedPermute.apply(tokens, probs, routing_map, tokens_per_expert, splits)
    )
    return permuted_input, permuted_probs, row_id_map, row_id_indices
...
class _SmoothFusedUnpermute(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(
            row_id_map, row_id_indices, org_smooth_scale, token_count_per_expert
        )
        ctx.input_requires_grad = permuted_tokens.requires_grad
        ctx.num_experts = n_experts
        ctx.restore_shape = restore_shape
    ...
    def backward(ctx, grad_output):
        ...
        # import pydevd
        # pydevd.settrace(suspend=False, trace_only_current_thread=True)
        permuted_grad_data, permuted_grad_scales = (
            triton_batch_smooth_fused_permute_with_indices(
...
def smooth_fused_unpermute(
    ...
    output = _SmoothFusedUnpermute.apply(
        permuted_tokens,
        row_id_map,
        row_id_indices,
        org_smooth_scale,
...
class _MXFP8Permute(torch.autograd.Function):
    ...
    def forward(
        ...
        row_id_map, reverse_row_id_map = triton_make_chunk_sort_map(
            split_sizes.reshape(-1, tokens_per_expert.size(0)),
            tokens_per_expert_cpu.tolist(),
        )

        x_q, x_s, xt_q, xt_s, permuted_probs = triton_batch_mxfp8_permute_with_indices(
            inp,
            tokens_per_expert,
            row_id_map,
            tokens_per_expert_cpu.tolist(),
