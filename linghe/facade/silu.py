...
class BlockSiluFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, quantizer, grad_quantizer, cls, limit):
        ...
        x_q, x_scale, xt_q, xt_scale = triton_silu_and_block_quant_forward(
            input_view, round_scale=round_scale, limit=limit
        )
    ...
    def backward(ctx, grad_output):
        ...
        x_q, x_scale, xt_q, xt_scale = triton_silu_and_block_quant_backward(
            grad_output_view, input_view, round_scale=round_scale, limit=ctx.limit
        )
        output = ctx.cls(
            shape=ctx.shape,
...
def block_batch_weighted_silu_impl(
    input,
    weights,
    counts,
    splits,
    ...
    assert input.ndim == 2
    output = BlockBatchWeightedSiluFunction.apply(
        input,
        weights,
        counts,
        splits,
...
class MXFP8SiluFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        x_q, x_scale, xt_q, xt_scale = triton_silu_and_mxfp8_quant_backward(
            grad_output, input, limit=ctx.limit
        )
        output = ctx.cls(
            shape=ctx.shape,
            dtype=grad_output.dtype,
...
class MXFP8BatchWeightedSiluFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        (x_q, x_scale, wgrad, xt_q, xt_scale) = (
            triton_batch_weighted_silu_and_mxfp8_quant_backward(
                grad_output, input, weights, counts, splits=ctx.splits, limit=ctx.limit
            )
        )
        output = ctx.cls(
            shape=ctx.shape,
...
def mxfp8_batch_weighted_silu_impl(
    input,
    weights,
    counts,
    splits,
    ...
    assert input.ndim == 2
    output = MXFP8BatchWeightedSiluFunction.apply(
        input,
        weights,
        counts,
        splits,
...
class SmoothSiluFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, quantizer, grad_quantizer, cls):
        ...
        ctx.cls = cls
        ctx.save_for_backward(input)
        x_q, x_scale = triton_silu_and_smooth_quant_forward(
            input,
    ...
    def backward(ctx, grad_output):
        ...
        x_q, x_scale, xt_q, xt_scale = triton_silu_and_smooth_quant_backward(
            grad_output,
            input,
...
class SmoothBatchWeightedSiluFunction(torch.autograd.Function):
    ...
    def forward(ctx, inputs, weights, counts, splits, quantizers, grad_quantizers, cls):
        ...
        x_q, x_scale = triton_batch_weighted_silu_and_smooth_quant_forward(
            inputs, weights, counts, smooth_scale=smooth_scales, round_scale=round_scale
        )
        output = cls(
            shape=x_q.shape,
            dtype=inputs.dtype,
...
def smooth_batch_weighted_silu_impl(
    inputs, weights, counts, splits, quantizers, grad_quantizers, cls
):
    # TODO(nanxiao): support recomputation
    assert inputs.ndim == 2
    output = SmoothBatchWeightedSiluFunction.apply(
        inputs, weights, counts, splits, quantizers, grad_quantizers, cls
    )
    return output
