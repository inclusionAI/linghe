...
class MXFP8RMSNorm(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, rms, eps, quantizer, cls, is_recomputing):
        ...
        if is_recomputing is None:
            ...
            output_mode = 0
        x_q, x_scale, output_rms, xt_q, xt_scale = triton_rms_norm_and_mxfp8_quant_forward(input, weight.data, rms=rms,
                                                                                           eps=eps,
                                                                                           output_mode=output_mode
                                                                                           )
...
def mxfp8_rms_norm(input, weight, rms, quantizer, cls, eps=1e-6,
                   is_recomputing=None
                   ):
    # input: [length,bs,dim]
