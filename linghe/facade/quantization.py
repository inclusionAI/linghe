...
class BatchSmoothQuantize(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        y_q, y_scale = triton_batch_smooth_quant(
            grad_output,
            smooth_scale_invs,
            token_count_per_expert,
            reverse=True,
        ...
        transpose_smooth_scale_invs = [
            x.transpose_smooth_scale_inv for x in grad_quantizers
        ]
        transpose_smooth_scale_invs = torch.cat(transpose_smooth_scale_invs, 0)

        yt_q, yt_scale = triton_batch_transpose_smooth_quant(
            grad_output,
            transpose_smooth_scale_invs,
            token_count_per_expert,
            ctx.splits,
...
def batch_smooth_quantize(
    hidden_states, token_count_per_expert, quantizers, grad_quantizers, splits, cls
):
    return BatchSmoothQuantize.apply(
        hidden_states, token_count_per_expert, quantizers, grad_quantizers, splits, cls
    )
...
# dx = y @ wT
# dwT = yT @ x
def triton_smooth_quant_activation(
    x,
    smooth_scale,
    x_q=None,
    x_scale=None,
