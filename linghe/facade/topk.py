...
class TopkFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output, grad_indices):
        ...
        grad_input = triton_topk_backward(grad_output, indices, ctx.shape[-1], dim=ctx.dim)
...
class GroupTopkScoreFunction(torch.autograd.Function):
    """"""
    @staticmethod
    def forward(ctx, x, topk, expert_bias, num_groups, group_topk, scaling_factor, score_function):
...
def group_topk_score(x,
    ...
    """
    return GroupTopkScoreFunction.apply(x, topk, expert_bias, num_groups, group_topk, scaling_factor, score_function)