...
class TopkFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output, grad_indices):
        ...
        grad_input = triton_topk_backward(
            grad_output, indices, ctx.shape[-1], dim=ctx.dim
        )
...
class GroupTopkScoreFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        probs, routing_map, counts = triton_group_topk_score_forward(
            x,
            topk,
    ...
    def backward(ctx, grad_output, grad_map, grad_counts):
        ...
        grad_input = triton_group_topk_score_backward(
            grad_output, x, routing_map, scaling_factor=ctx.scaling_factor
        )
        return grad_input, None, None, None, None, None, None


def group_topk_score(
    x,
    topk,
    expert_bias=None,
    num_groups=32,
    ...
    """
    return GroupTopkScoreFunction.apply(
        x, topk, expert_bias, num_groups, group_topk, scaling_factor, score_function
    )
