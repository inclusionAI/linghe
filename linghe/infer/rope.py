...
@triton.jit
def varlen_qk_norm_and_half_rope_kernel(
    qkv_ptr,
    q_norm_weight_ptr,
    k_norm_weight_ptr,
    freqs_ptr,
    ...
    if B == 1:
        qr = tl.reshape(
            tl.permute(
                tl.flip(tl.permute(tl.reshape(q0, (PH, 2, d)), (0, 2, 1)), dim=2)
                * signs,
                (0, 2, 1),
            ),
            (PH, D),
        ...
        qr = tl.reshape(
            tl.permute(
                tl.flip(tl.permute(tl.reshape(q0, (G, 2, d)), (0, 2, 1)), dim=2)
                * signs,
                (0, 2, 1),
            ),
            (G, D),
    ...
    if B == 1:
        kr = tl.reshape(
            tl.permute(
                tl.flip(tl.permute(tl.reshape(k0, (ph, 2, d)), (0, 2, 1)), dim=2)
                * signs,
                (0, 2, 1),
            ),
            (ph, D),
        ...
        kr = tl.reshape(
            tl.permute(
                tl.flip(tl.permute(tl.reshape(k0, (1, 2, d)), (0, 2, 1)), dim=2)
                * signs,
                (0, 2, 1),
            ),
            (1, D),
...
def triton_varlen_qk_norm_and_half_rope(
    qkv,
    q_norm_weight,
    k_norm_weight,
    freqs,
...
def mla_rope_kernel(
    ...
    sin = tl.load(freqs_ptr + pos * D + d + tl.arange(0, D) % d).to(tl.float32)
    if INTERLEAVE:
        cos = tl.reshape(tl.trans(tl.reshape(cos, (2, d))), (D,))
        sin = tl.reshape(tl.trans(tl.reshape(sin, (2, d))), (D,))
    ...
    if INTERLEAVE:
        qr = tl.reshape(tl.flip(tl.reshape(q, (h, d, 2)), dim=2) * signs, (h, D))
    else:
        qr = tl.reshape(
            tl.permute(
    ...
    if SINGLE:
        if hid == 0:
            k = tl.load(k_ptr + pid * k_stride_0 + tl.arange(0, D)).to(tl.float32)

            if INTERLEAVE:
                kr = tl.reshape(tl.flip(tl.reshape(k, (d, 2)), dim=1) * signs, (D,))
            else:
                kr = tl.reshape(
                    tl.permute(
                        tl.flip(tl.permute(tl.reshape(k, (2, d)), (1, 0)), dim=1)
                        * signs,
                        (1, 0),
                    ),
                    (D,),
            ...
            k = k * cos + kr * sin
            tl.store(k_ptr + pid * k_stride_0 + tl.arange(0, D), k)
    else:
        ...
        if INTERLEAVE:
            kr = tl.reshape(tl.flip(tl.reshape(k, (h, d, 2)), dim=2) * signs, (h, D))
        else:
            kr = tl.reshape(
                tl.permute(
                    tl.flip(tl.permute(tl.reshape(k, (h, 2, d)), (0, 2, 1)), dim=2)
                    * signs,
                    (0, 2, 1),
                ),
                (h, D),
...
def triton_mla_rope(q, k, freqs, position_ids, interleave=True):
    """
    apply MLA-type rope
    Args:
