...
@triton.jit
def group_rms_norm_gate_kernel(
    x_ptr,
    gate_ptr,
    weight_ptr,
    out_ptr,
    ...
    offs = pid * DIM + tl.arange(0, GROUP_SIZE)[:, None] * D + tl.arange(0, D)[None, :]
...
def triton_group_rms_norm_gate(
    x: torch.Tensor,
    gate: torch.Tensor,
    weight: torch.Tensor,
    eps=1e-6,
    ...
    group_rms_norm_gate_kernel[grid](
        x, gate, weight, out, eps, dim, d, group_size, num_stages=3, num_warps=4
    )
    return out


@triton.jit
def block_group_rms_norm_gate_kernel(
    x_ptr,
    gate_ptr,
    weight_ptr,
    out_ptr,
    ...
    weight = tl.reshape(weight, [GROUP_SIZE, d])
    offs = pid * D + tl.arange(0, GROUP_SIZE)[:, None] * d + tl.arange(0, d)[None, :]
    ...
    tl.store(scale_ptr + pid + tl.arange(0, B) * PM, tl.reshape(scale, (B,)))
...
def triton_block_group_rms_norm_gate(
    x: torch.Tensor,
    gate: torch.Tensor,
    weight: torch.Tensor,
    eps=1e-6,
