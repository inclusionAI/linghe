...
def bench_topk(M=4096, N=256, k=8):
    ...
    values_ref, indices_ref = torch.topk(logits, k)
    # print(f'{indices_ref[0]}')
    values, indices = fused_topk(logits, k)
    # print(f'{indices[0]}')

    ref_time = benchmark_func(torch.topk, logits, k, ref_bytes=M * N * 4)
    benchmark_func(fused_topk, logits, k, ref_time=ref_time, ref_bytes=M * N * 4)
