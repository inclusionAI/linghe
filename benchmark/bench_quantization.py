...
def bench_block_quantization(M=8192, N=4096, round_scale=True):
    ...
    qw = weight_quantizer.make_empty(
        (N, N), dtype=dtype, device=device, requires_grad=False
    )
...
def bench_batch_mxfp8_quant(M=4096, N=4096, n_experts=32, bench=False):
    ...
    quantizers = [
        MXFP8Quantizer(fp8_dtype=tex.DType.kFloat8E4M3) for _ in range(len(splits))
    ]
    ...
    x_q, x_scale, xt_q, xt_scale = triton_batch_mxfp8_quant(
        x, token_count_per_expert, splits, output_mode=2
    )
