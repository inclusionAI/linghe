...
def split_permute_pad_quantize(x, probs, mask_map, fp8_padding, out_tokens, token_count_per_expert_list):
    M, N = x.shape
    n_experts = mask_map.size(1)
    row_id_map = triton_permutation.make_row_id_map(mask_map, M, n_experts)
    output, permuted_scale, permuted_probs = triton_permutation.permute_with_mask_map(x,
                   row_id_map, probs, None, M,
                   n_experts, out_tokens, N, 1)
    output,_ = fp8_padding(output, token_count_per_expert_list)
    permuted_probs,_ = fp8_padding(permuted_probs.view(-1,1), token_count_per_expert_list)
    ...
    qx = quantizer.make_empty(output.shape, dtype=x.dtype, device=x.device, requires_grad=False)
    qx = quantizer.update_quantized(output, qx)

    return qx, permuted_probs

def fused_permute_pad_quantize(x, probs, mask_map, token_count_per_expert,  token_count_per_expert_list):
...
def bench_permute_pad_quantization(M=4096, N=4096, n_experts=32, topk=2):
    ...
    split_permute_pad_quantize(x, probs, mask_map, fp8_padding, out_tokens, token_count_per_expert_list)
    fused_permute_pad_quantize(x, probs, mask_map, token_count_per_expert, token_count_per_expert_list)

    ref_time = benchmark_func(split_permute_pad_quantize,
                   x, probs, mask_map, fp8_padding, out_tokens,token_count_per_expert_list)
    benchmark_func(fused_permute_pad_quantize,
                   x, probs, mask_map, token_count_per_expert, token_count_per_expert_list,
                   ref_time=ref_time)
...
if __name__ == '__main__':
    ...
    bench_triton_unpermute_with_mask_map(M=8192*4, N=2048, n_experts=32, topk=2) 
