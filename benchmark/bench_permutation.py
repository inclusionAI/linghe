...
def split_permute_pad_quantize(
    ...
    output, permuted_scale, permuted_probs = triton_permutation.permute_with_mask_map(
        x, row_id_map, probs, None, M, n_experts, out_tokens, N, 1
    )
    output, _ = fp8_padding(output, token_count_per_expert_list)
    permuted_probs, _ = fp8_padding(
        permuted_probs.view(-1, 1), token_count_per_expert_list
    )
    ...
    qx = quantizer.make_empty(
        output.shape, dtype=x.dtype, device=x.device, requires_grad=False
    )
...
def fused_permute_pad_quantize(
...
def bench_permute_pad_quantization(M=4096, N=4096, n_experts=32, topk=2):
    ...
    split_permute_pad_quantize(
        x, probs, mask_map, fp8_padding, out_tokens, token_count_per_expert_list
    )
    fused_permute_pad_quantize(
        x, probs, mask_map, token_count_per_expert, token_count_per_expert_list
    )
