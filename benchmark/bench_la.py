...
def bench_la(B=1, S=4096, H=32, D=128):
    query = torch.randn(B, S, H, D, device='cuda', dtype=torch.bfloat16, requires_grad=True)
    key = torch.randn(B, S, H, D, device='cuda', dtype=torch.bfloat16, requires_grad=True)
    value = torch.randn(B, S, H, D, device='cuda', dtype=torch.bfloat16, requires_grad=True)
    ...
    benchmark_func(chunk_lightning_attn, query, key, value, layer_idx=1, num_layers=20, output_final_state=True)
