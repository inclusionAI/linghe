...
def bench_mla_rope(B=2, L=4096, H=32, rope_theta=10000.0, transpose=True):
    ...
    k_pos_emb = (
        torch.randn(L, B, 64 + 512, dtype=dtype, device=device)[:, :, :64]
        .view(L, B, 1, 64)
        .requires_grad_()
    )
...
def bench_varlen_mla_rope(
    lengths=[2048, 2048], H=32, rope_theta=10000.0, cp_size=1, cp_rank=0, stride=True
):
    dtype = torch.bfloat16
    device = "cuda:0"
    q = torch.randn(
    ...
    if stride:
        k_pos_emb = torch.randn(
            sum(lengths) // cp_size, 576, dtype=dtype, device=device
        )
        k_pos_emb = (
        ...
        k_pos_emb = torch.randn(
            sum(lengths) // cp_size, 1, 64, dtype=dtype, device=device
        ).requires_grad_()
...
if __name__ == "__main__":
    ...
    bench_varlen_mla_rope(
        lengths=[444, 503, 434, 433, 472, 483, 557, 770],
        H=16,
        rope_theta=10000.0,
        cp_size=1,
        cp_rank=0,
